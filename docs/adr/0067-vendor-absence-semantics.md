...
- Status: Accepted (2026-08-24) · **Amended (2026-08-27)** — D3 的「待查」项 `last` / `prev_close` 经 vendor 书面答复结案，判据落在新增的 §D2a；**决策本身未变**（本 ADR 一直要求「求证后再定」，这是执行它、不是改它）
...
### D2a — 但「成对」是**手段不是教条**：反向约束消失时，单列判据才是对的

D2 之所以要求成对，唯一理由是 OPRA 那条反向约束——**零价可能是合法买价**，所以单看价格会静默吃掉真实报价。凡是没有这条反向约束的列，硬套成对判据反而是错的（`last` 根本没有伴生的 size 可配对）。

`last` / `prev_close` 就是这一类。富途官方于 [py-futu-api#258](https://github.com/FutunnOpen/py-futu-api/issues/258)（2026-08-27）**书面**答复：

- SDK 侧 `last_price = record.basic.curPrice`、`prev_close_price = record.basic.lastClosePrice` **直接透传**，无空值转换；
- OpenD 侧 `QotRealTimeData.cpp` 新建缓存条目时默认 `set_curprice(0) / set_lastcloseprice(0)`，**`0` 是「无价格数据」的占位**；
- **「期权成交价恒为正值（最小价位 > 0），接口返回的 `0` 一律表示『无最后成交价』，不存在『真实成交价恰为 0』的情况」**。

⇒ 判据出处从 **D2 那种「从数据反推」升级为「vendor 书面契约」**，正是 D5 第 3 问要的最强那一档；它不会像反推判据那样「过期时不报错」。

**两个曾被当成「哨兵理论破裂」的形态，官方给了解释，故一并归一：**

| 形态                        |  实测 | 官方解释                                     |
| --------------------------- | ----: | -------------------------------------------- |
| `prev_close = 0 ∧ last > 0` | 81 行 | 新挂牌合约**首日无前收盘**                   |
| `last = 0 ∧ volume > 0`     |  2 行 | 首个交易日「价格字段尚未填充」，**仍是无值** |

⚠️ 官方称「后续交易日价格即恢复正常」，但**救不了已落的行**——供应方不提供历史期权快照。归一成 null 才是诚实表示：我们确实不知道那天的成交价。
⚠️ 官方对「为何首日有成交却无价」只答「尚未填充」，未给机制根因；我们的数据把它框在「首日 + 从未成交」两档内（5509/5511 是 `volume = 0 ∧ open_interest = 0`），够用但不是封闭解释。

📌 **`underlying_spot` 一并纳入**：它取自标的行的 `last`，而下游对 `spot === null` 有守卫、对 `spot === 0` **没有** ⇒ 哨兵会一路算进 moneyness / 有效成本。
...
| **已求证**       | `last` / `prev_close` —— vendor **书面确认** 0 即无值（见 D2a）                             | 归一为 null，**单列判据**（`tradedPriceOrNull`）。🚫 别再搬回本行上方的「不可判定」档    |
