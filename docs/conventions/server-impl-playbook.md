...
新 server use case 起手**对照样板文件**（文件头有 `// GOLDEN SAMPLE` banner）。两档按复杂度选：**简单档**（默认 copy；干净单 ctx CRUD：读 row → 校验 → update → 投影返回）→ `account/update-display-name.usecase.ts`（flat 内构 + 贫血 Prisma row + `*.rules.ts` 纯函数 + 直注 `PrismaService` + `@map`）；**跨 ctx 档**（编排多 bounded context 的 user-facing 流程）→ `auth/phone-sms-auth.usecase.ts`（+ 写半段 `account/commit-phone-login.usecase.ts`；两段式 Inspect/Commit 护城河 + `CROSS-CONTEXT-SYNC` 注入 per [ADR-0043](../adr/0043-server-flat-module-paradigm.md) + 并发注册 P2002/P2034 双形态见 § P3）。**全档样板「学什么」明细 → [golden-sample-registry](golden-sample-registry.md)「Server CRUD / Server 跨 ctx 编排」行。**

- **⚠ negative（样板内就地标注「勿照抄」）**：跨 ctx 档的 **timing pad + 反枚举折叠**是 public auth 探测面专属（§ S1），标准跨 ctx use case 不需要。
- **默认并发范式**不在样板里复述 → 见 § P1（conditional UPDATE + affected-count）；跨行聚合不变性 → § P7 + [ADR-0046](../adr/0046-cross-row-invariant-concurrency.md)。
- 跨端配对：简单档 server `update-display-name` ⇄ mobile `name-edit.tsx`（同一 display-name feature 两端，可对照通读全栈）。
