...
>
> **引用不复述**：目录结构 / 凭据存储 → [fe-directory-structure.md](fe-directory-structure.md)；UI mockup-first flow → [sdd.md](sdd.md) § 前端 UI 工作流；Metro `.js` extensionless 陷阱 → memory `metro_web_cannot_resolve_js_extension_imports`（ESLint `no-restricted-syntax` 已机械拦）；测试分层 → memory `mono_mobile_test_layering`（vitest=logic-only / Playwright=UI）。
>
...
**分层样板**（文件头有 `// GOLDEN SAMPLE` banner，起手对照）：4 铁律逻辑权威 → `apps/mobile/src/auth/use-login-form.ts`；最小标准编辑表单屏（默认 copy）→ `apps/mobile/app/(app)/settings/account-security/name-edit.tsx`。⚠ 进阶参照 `app/(auth)/login.tsx` 的富状态机 + FROZEN 拦截 modal + success overlay 是 **login 专属**，标准表单别整屏照抄。全档样板（含边界）见 [registry](golden-sample-registry.md)「表单逻辑 / 编辑表单屏」行。

- **实证锚**：login / cancel-deletion（004 #198）；memory `rhf_form_standard_login_golden_sample`。
...
## 3. Claude Design mockup 2 段模板（design 先行）

适用所有 UI feature（mockup-first，per [sdd.md](sdd.md) § 前端 UI 工作流）。产出 = **HTML preview baseline**（非最终 RN），再视觉翻译为 RN（复用 `~/theme` token + `~/ui` + reanimated）。**0 新 token**（视觉资产已稳定）。

> **生成走 [`/mockup-gen NNN`](../../.claude/commands/mockup-gen.md)**：自动从 spec 组装下方两段 prompt + 驱动 Claude Design 生成 + 落 `design/`，零 copy/paste。本 §3 是 prompt **模板 SoT**；project 映射机制见 [design-system-mapping.md](design-system-mapping.md)。

mockup-prompt 两段结构（`/mockup-gen` 自动组装，下为模板）：
...
per [Constitution §V](../../.specify/memory/constitution.md)：跨端 feature 的 mobile 侧除 ① hermetic UI e2e（点通）外，必落 ② **契约冒烟**——node 层、无浏览器，用**生成的 `@nvy/api-client`** 打 testcontainers 真 server 跑一条 happy-path，验**契约对齐**（URL/method/序列化/响应解封/错误码）+ **基建**（真 token / 真落库 / 真状态机）。这是 hermetic mock（mock 即假设契约）与 server IT（不经生成客户端）**都覆盖不到的缝**。
...
- **样板**：`apps/mobile/e2e/contract-smoke/broker-account.contract.ts`（012 pilot：bind 201 → list raw 读回证落库 → dup 真 409 → cleanup）。
- **跑**：`RUN_REAL_BACKEND_SMOKE=true nx run mobile:contract-smoke`（本地前置 = 杀 :3000 + env 前缀，见 [local-verification](local-verification.md) §1-§2；CI 进 `e2e-real-backend.yml` nightly 软信号）。
...
## 6. hermetic e2e mock 纪律（mock 写依赖方契约，不写消费方调用序）

Playwright hermetic e2e（`route.fulfill` 拦截 server 端点）的 mock **必须模型依赖方（server）的契约**——「无条件镜像服务端 canonical 状态」，**不得**把消费方（客户端 hook）当前怎么取数的时序假设编码进去。

- **stateful mock = 单一 canonical 状态 + 纯函数 handler**：维护一份镜像服务端不变量的状态（如 append-only 的 `turns[]`），`POST` 追加、`GET` **无条件**返回当前状态。route handler 是 `(request, 累积服务端状态) → response` 的纯函数，与「客户端何时/为何调它」无关。
- **禁按测试编排标志 / 客户端信号分支**：`turns: briefGenerated ? turns : []` 这种把「客户端何时 invalidate / refetch」写进 server mock 的条件门是**反模式**——真 `GET /sessions/{id}` 不知道也不关心 brief 是否已生成、不会因此分支。判断法：**「真 endpoint 会因为这个条件分支吗？」** 条件若是客户端概念（UI flag / 缓存失效时机 / 取数序）→ 不该进 server mock；只有服务端事实（已落什么 / auth / ownership）能进。
- **为什么**：mock 一旦编码「消费方当前调用序」而非「依赖方契约」，客户端行为一改（如给 hook 加 `invalidateQueries` → 多一次 refetch）就破，即便真 server 永远不会错。**越笨的 mock（无条件镜像 canonical 状态）越稳**；任何返回派生 / 条件视图的 handler 都是 smell——从 canonical 状态派生，别按客户端信号 gate。
- **typecheck 拦不住，靠测试网**：`briefGenerated ? turns : []` 两分支都是 `Turn[]`，类型完全合法——这是**行为 / 时序耦合**非形状错，编译器零感知。把 mock 内部状态 typed 成生成的响应 DTO（`@nvy/api-client` model）有助于「持一个真实状态对象再返」，但抓不到「错时刻返数据」。真正的网 = ① 改**共享 hook / util 行为** → 跑**全 `runtime-smoke`**（非只跑被改 spec；blast radius 是整套 e2e，sibling spec 的 mock 可能把旧行为写死了）② 关键行为有 `[Contract-Smoke]` 断言（打真 server，抓 mock↔契约 drift）。
- **实证锚**：032 FU-1 / FU-1a —— clarify hook `onDone` 加 `invalidateQueries(sessionControllerGet)`（修热重进陈旧空 turns）后，`ideation-clarify.spec.ts` 详情 GET 的 `briefGenerated ? turns : []` 守卫让 invalidate 触发的重取返空 turns、hydrate 把刚流式的反问气泡冲掉（CI runtime-smoke 红，typecheck 全绿）；改无条件返 `turns`（镜像 append-only 真相）即修。单跑被改 spec 抓不到，全 runtime-smoke 才暴露。
...
- **JS-only 根治（不依赖原生 patch / 不重建 dev-client）**：把目标屏**收进 tab 自带嵌套 stack**（`app/(app)/(tabs)/<feat>/{_layout,index,[id]}`）——进 tab = 普通切换、列表→详情 = 同 navigator push，从结构上消除跨 stack 重挂。
  - 详情屏隐藏 tab 栏：`getFocusedRouteNameFromRoute(route) === '[id]'` → `tabBarStyle: { display: 'none' }`，且 **options 只对该 tab 用函数形式**。⚠️ **门控 tab 的 `href: null` 必须留在静态 options 对象**——expo-router 布局期读静态 `href` 决定 tab 是否渲染，options 用函数形式时 `href` 不被采纳 → 门控失效（公开版漏出 gated tab，e2e-public 抓）。
...
本仓**约定根 `app/_layout.tsx` 不全局挂 `GestureHandlerRootView`**（仅 `QueryClient > SafeArea > Keyboard > Stack`）—— 凡用手势（`GestureDetector` / `SwipeRow` / `LongPressMenu` / `DraggableList`）的屏**各自自套一层** `<GestureHandlerRootView style={{ flex: 1 }}>`。先例：`ideation/index.tsx`、portfolio 三屏（broker / watchlist / group-management）、`profile-image/image-viewer`、`chat-drawer`。
...
- **实证锚**：#606（036 标注画布 `image-annotate.tsx`）初版漏套、注释还误写「根已挂」→ 标注真机一进即崩；按约定补 GHRV（commit da90a8de）。
...
- **修法**：相册 pick **不 gate** 该权限 —— 系统 picker 让用户显式选图、回传 scoped `content://` URI 授权，app 本不需持有读权限；直接调 `launchImageLibraryAsync`。**相机**仍需 `requestCameraPermissionsAsync`（CAMERA 真需要且可授予）。已修 `use-ideation-attachments.ts` + `use-profile-image-upload.ts`（commit 8990b1b4）。
