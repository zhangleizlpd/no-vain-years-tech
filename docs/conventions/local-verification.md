...
两段旧前缀的退役理由留痕在 §3 末尾「已退役的说法」表；`NX_DAEMON=false` 只在撞上本地成片 spurious TS6059（而 CI 绿、单独 `tsc --build` 也绿）时作 workaround，别默认带、别去改 tsconfig。

`MARKETDATA_PROVIDER` 的耐久机制（显式给 `=live` 时仍成立）：`marketdata.config.ts` 蓄意不给 `FUTU_SHIM_URL` / `FUTU_SHIM_TOKEN` `.default()`，而 `.env` 的 URL 现为空串 ⇒ **显式 `live` 的 boot 必炸**（本地 live 联调这条路当前蓄意不可用）。复跑（需 dev 栈起着：`docker compose -f docker-compose.dev.yml up -d --wait`；cwd 必须是 `apps/server`，否则 `.env` 压根不加载，会先死在 `jwtSecret`）：
...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| server 单测 / IT（单文件）                       | `nx test server <file>`                                                                     | 🚨 **必须走 nx，`cwd=apps/server`；不是 `vitest --root`**（`--root` 找不到 prisma schema，🟢 实证；hook 拦）                                                                                               |
| **server 单测全量（不需要 Docker）**             | `nx test server -- --project unit`                                                          | 秒级，零容器。🟢 实证：把 `DOCKER_HOST` 指到死端口仍全绿，同一 env 打任一 IT 则报 `Could not find a working container runtime strategy`（`src/` 下的容器 spec 迁出 `unit` 后成立，`check-test-size` 守着） |
| server 全量                                      | `nx test server --skip-nx-cache`                                                            | 分钟级以内；env-gated 真 vendor IT 默认 skip                                                                                                                                                               |
...
| 契约冒烟（真 server）                            | `RUN_REAL_BACKEND_SMOKE=true nx run mobile:contract-smoke`                                  | **无需任何 env 前缀**（harness 恒钉，见 §3 退役表）；本地先**杀掉占用 :3000 的进程**                                                                                                                       |
| OpenAPI 导出                                     | `nx run server:export-openapi`                                                              | 🚨 必走 canonical `node dist/main.js`（非 `dump.mjs`）；**需 Redis 可达**（DB 不必可达，`DATABASE_URL` 合法即可；`nx test server` 两者都不需要）；见下                                                     |
...
| 现象                                                                                                                     | 等级        | 真因                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 处置                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------------------------------------ | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 一片 **TS6305**，可能夹带一条**语义型** `TS2741`/`TS2554`                                                                | 🟢 实证     | `apps/server/src/generated/prisma` 是**陈旧构建产物**。触发路径之一 = **切分支**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `pnpm -C apps/server exec prisma generate` 后重跑。⚠️ 那条语义错误会**一起消失** —— 别拿它去追「谁的 PR 破坏了类型」（2026-08-02：`dimension-executor.ts` 的 TS2741 看着像旧仓 #789/#792 语义冲突，实为陈旧 client）。<br>🔻 另一条触发路径「改完 schema 又撤回」危险得多，见 [`.claude/rules/migration-rules.md` §0](../../.claude/rules/migration-rules.md)（canonical，本表不复述）                                                                                                                                                      |
| 大批 IT 红，日志里全是 **`ECONNREFUSED`**                                                                                | 🟢 实证     | **不是缺 env。** 看目标端口：若是 `127.0.0.1:57058` 这类**临时高位端口**，那是 **Testcontainers 映射到宿主的端口** ⇒ 真因是并行 IT 各自的 Redis 容器（蓄意每文件独立）+ 少数仍自起容器的 spec 一起起，容器起不来 / 连不上                                                                                                                                                                                                                                                                                                                                                                                                          | 降并发或让 docker 喘口气后重跑；**与任何 env 前缀无关**；PG 已走共享容器 + template 克隆（矩阵 §1），Redis 蓄意不共享                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 全量并行时某条 IT 红，且耗时异常长                                                                                       | 🟢 实证     | **负载敏感 flake**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **单跑该 spec 复验**再下结论。2026-08-02：`anonymize.us7.it.spec.ts` 全量下 36.9s 红、单跑 3.7s 绿 4/4；同日 `server:test` 在与 mobile 三个重任务并发时红、单独重跑全量全过                                                                                                                                                                                                                                                                                                                                                                 |
...
| **frontmatter 检查报「全部字段 `undefined`」**（`spec-frontmatter-check` / `adr-frontmatter-check`，而文件肉眼完全正常） | 🟠 单次观察 | **commit hook 并行竞态，不是文件写错。** `pre-commit` 下所有 hook 并行跑（`lefthook.yml` `parallel: true`；两个 frontmatter check 由 preset 经 `extends` 并入同一 hook）⇒ `format`（`prettier --write` + `stage_fixed`）正在重写这个 `.md` 时，checker 可能读到写了一半的内容，gray-matter 解不出 frontmatter，于是**每个字段都报 undefined**。窗口只在「prettier 这次真要改这个文件」时存在（hook 输出该文件名且**没有** `(unchanged)` 标记）<br>🔻 **为什么不是 🟢**：红臂是竞态、**不可按需复现**，只有「串行跑绿 / 原样重跑绿」两条反向对照（未混淆其它变量，缺的就是红臂可复现性）⇒ 按 §0 第 1 条**不得**据此加 hook 拦截规则 | **原样重跑 `git commit`** —— 第一轮 prettier 已把文件规范化并 re-stage，第二轮无格式化工作 ⇒ 窗口消失。<br>判它是不是假红（任一条即可）：① 红的是**全部**字段而非某字段值不合法（真写错只红一两条）；② hook summary 里 `format` 与该 checker 耗时几乎相同；③ 手工串行跑 `pnpm exec tsx scripts/check-spec-frontmatters.ts <那个文件>` 当场绿。<br>🚫 **别去改 frontmatter 内容「修」它** —— 文件本来就是对的，改了反而把对的写坏（🟠 2026-08-31 实撞：070 spec.md 新增两段表格触发 prettier 重排，11 个字段一起 undefined）                 |
| **逐文件 diff 审计里，某个你根本没碰过的文件显示「删了一大批行」**                                                       | 🟢 实证     | **`git diff` 的两点 `A..B` 比的是两个端点的当下差异** —— base 在你分叉之后前进过的 commit 会被**反向计入**，看起来就像你把它们删了。🚨 根在 subcommand 间的**语义不对称**：同一个两点写法，`git log A..B` 是「B 可达而 A 不可达的 commit」（正是你要的答案），`git diff A..B` 却不是。写法一样、答案一对一错，所以照着 `log` 的直觉去写 `diff` 必错                                                                                                                                                                                                                                                                                | 审计「**本分支改了什么**」一律用**三点** `origin/main...HEAD`（= merge-base→HEAD，与 PR 展示口径一致）；两点只在你真想比「两个端点的当下状态」时才用。<br>判它是不是这条（任一即可）：① diff 里出现你没碰过的文件；② 删除行数异常大且全是别人的代码；③ `git log HEAD..origin/main` 非空 ⇒ base 已前进，两点必然失真。<br>（🟢 2026-09-06 075 T007 实撞：`db-option-chain-discovery.adapter.spec.ts` 两点报 `0 added / 170 deleted`、三点为空，真凶是 main 侧 `232636f4` / `3ca1e478`；同一分支 `git log origin/main..HEAD` 给的 15 是对的） |
| **用例全绿但 `Errors 1 error`**，报 `Unhandled Rejection: … ECONNREFUSED 127.0.0.1:6399`，单跑恒绿                       | 🟢 实证     | **bullmq 5.x 关停竞态，不是代码回归。** 测试 `REDIS_URL` 钉无人监听端口，`OptionsdeskModule` 连带起 marketdata 队列 `Worker` / `QueueEvents` 空连；`RedisConnection.close()` 先 `removeAllListeners()`，其客户端恰在 `connecting` 态（时序相关 ⇒ 负载下偶发）时，构造期 `initializing.catch(emit('error'))` 对零监听器抛出。<br>🟢 强制臂：spec 顶部临时包 `Worker.prototype.close`，等 `blockingConnection` 客户端进 `connecting` 再调原方法，单跑 `nx test server test/integration/optionsdesk-046.thermometer.it.spec.ts --skip-nx-cache` ⇒ 未置位 `Errors 1 error`；置位后 `RedisConnection` 零构造（2026-09-14）              | 2026-09-14 起已在测试侧规避（相关 IT 设 `MARKETDATA_WORKER_DISABLED`，顺带消掉每文件 `QueueEvents.close()` 吃满的关停超时）；若再现先查是否有新 IT 漏设。<br>库侧根治 = 升级 bullmq ≥ 6.0.0（跟踪 #407；上游 v6 发布提交 `e1f86effc5` 修了 `RedisConnection` 构造期 `initializing.catch` 的零监听器 emit），5.x 截至 5.81.5 未修（2026-09-14 核）。<br>🚫 别全局吞 `unhandledRejection` / `dangerouslyIgnoreUnhandledErrors`                                                                                                                |
...
| 「跑 test / smoke / export-openapi 必带 `MARKETDATA_PROVIDER=mock`」 | 🟢 **实证退役**             | 双层化解：① 关键 env 已烘进属主 —— vitest `test.env` / `export-openapi` target env / contract-smoke harness 恒钉（含 `CODE_INDEX_PROVIDER=fake`、`OSS_PUBLIC_BASE_URL=''`）；② `apps/server/.env` 的 dev 默认已改 `mock` —— 「`.env` 就是 live 且无 FUTU 真值」的旧前提不复存在（旧仓 #796 / #803 / #830） |
| 「必带 `NX_DAEMON=false`」                                           | 🟡 **间歇性未复现**         | 2026-08-03 daemon-on 全量门（含 `server:build`）多轮绿未复现；daemon 态问题无法一次性证伪 ⇒ 降级为撞上时的 workaround（判据见 §1），别默认带                                                                                                                                                               |
...
1. **新建的 `.ts` / `.spec.ts` / config 首跑必带 `--skip-nx-cache`** —— nx cache 对新文件会**假绿**。
   ...
   🚨 **测试在运行时读 project 之外的资产时同样必带**（🟢 2026-08-04 实证）：缓存 key 只看该 project 的 `inputs`，测试用 `readFileSync` 从仓内**别的目录**读进来的文件不在 hash 里（实例：`ops/jobs/marketdata-table-health.sql` ← `marketdata.table-health.it.spec.ts`）⇒ **只改那个文件时命中缓存直接返绿**。对照：把谓词判据从 AND 蓄意改成 OR，不带 flag **整轮全绿**；带 flag 当场翻红，且只有对应那条用例红。识别信号 = 日志里 `Nx read the output from the cache`。<br>⇒ 判断要不要带 flag，**不看「我改了哪些文件」，看「这个测试运行时会去读 project 外的东西吗」**。<br>🚨 **定向变异验证（proof-of-red）跑同样必带**（🟢 2026-08-30 069 实证）：脚本改完源文件**立刻**发 `nx test` 可能撞 daemon file-watch race ⇒ `existing outputs match the cache` 回放出**变异前的全绿**——变异「幸存」的结论必须 `--skip-nx-cache` 重跑确认；**红结果不受此害**（绿缓存放不出红），只有绿的幸存需要复核。识别信号同上 = 日志含 cache 回放字样。
2. **blast radius 大就跑全套，别跑单 spec**：改动触及被多处消费的共享面（`~/ui` 原语 / 容器 / design token）⇒ `runtime-smoke` **全套**；判据与实证见 [mobile-impl-playbook § 6](mobile-impl-playbook.md)。
3. **cwd 必须是 mono root**。用 `pnpm -C <dir>` / `git -C <dir>` / `--cwd`，**不要 `cd`** —— `cd` 的副作用会跨命令持续，后面的 `nx affected` 会在错的目录下解析出错的 affected 图（🟢 2026-08-02 实证）。（`bash-cwd-discipline` 是本机 user-level skill，非仓内。）
