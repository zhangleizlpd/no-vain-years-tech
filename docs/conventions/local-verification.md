...
| **「命令成功了」但结果不对**                                             | 🟢 实证 | **exit code 会说谎**：① 管道后的 `$?` 是**最后一段**的码 ② 后台/包装执行时 harness 报的码是**外层 wrapper** 的，不是 nx 的                                                                                                                                                                                                                                          | 判定成败**一律 grep 终态串**，不信单一 exit code：<br>`<cmd> > /tmp/v.log 2>&1; echo "EXIT=$?"`<br>`rg -n 'Successfully ran target\|Failed tasks' /tmp/v.log`<br>🚨 **`\| tail` / `\| head` 已被 hook 硬拦** —— 它同时吞掉退出码**和**失败证据（nx 的失败摘要在尾部，但失败的 spec 文件名在中间）                                                                                                                                                                                         |
