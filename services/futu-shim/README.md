...
| `GET /his-vol?code=US.PEP&start=&end=`                       | `his_volatility`      | Backs the `underlying_iv_history` backfill. Daily `iv` / `hv` / `underlying_price`, depth ≈3 years; the floor behaves like a **fixed epoch**, not a sliding window (see the `/his-vol` docstring for the measurements). Single-call span capped at **364 days**; wider is a **400**, never a clipped series — the SDK's `normalize_start_end_date(..., 364)` only derives a _missing_ side and will not reject an explicit over-wide window. Follows `page_req_key` to exhaustion; rate-gated once per request.                                                                                                                                                                                                                                                                                                                                                                  |
...
| `GET /trade/accounts`                                        | `trade_acc_list`      | **Read-only trade surface (082).** Returns `{trdmarket_auth, matched}` for the one account that is REAL ∧ ACTIVE ∧ authorised for HK or US. Any other match count is a **409** `{"error":"account_selection","matched":n}` — never "take the first". No part of `acc_id` leaves the shim, not even its last four digits: it is stripped from every row and kept out of every log line.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `GET /trade/positions?market=US`                             | `trade_position`      | `market` is `US` or `HK`. Positions of the selected account, queried with `refresh_cache=True`. Trade calls are bounded by `FUTU_TRADE_CALL_TIMEOUT_S` (**503** `trade_timeout`, context discarded) and by a non-blocking cap of 2 concurrent calls (**503** `trade_busy`) so the quote routes always keep waitress threads.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `GET /trade/deals?market=US&start=&end=`                     | `trade_deal_history`  | History deals over `start`..`end` (both required, span ≤ **90 days**, else **400**). When `end` ≥ the exchange's local today, today's deals (`trade_deal_today`) are merged in and de-duplicated by `deal_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `GET /trade/orders?market=US&start=&end=`                    | `trade_order_history` | Same shape as `/trade/deals`: history orders plus, when `end` reaches the exchange's local today, `order_list_query` (`trade_order_today`), de-duplicated by `order_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `FUTU_TRADE_CALL_TIMEOUT_S`  | `10`                  | Deadline per trade-query SDK call; a timeout discards the trade context and answers `503 trade_timeout`.                         |
