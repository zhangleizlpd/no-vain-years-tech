...
| `GET /trade/accounts`                                        | `trade_acc_list`      | **Read-only trade surface (082).** Returns `{last4, trdmarket_auth, matched}` for the one account that is REAL ∧ ACTIVE ∧ authorised for HK or US. Any other match count is a **409** `{"error":"account_selection","matched":n}` — never "take the first". The full `acc_id` never leaves the shim: it is stripped from every row and kept out of every log line.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
