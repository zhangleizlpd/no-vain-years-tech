...
export interface ChainReportResponse {
  ...
  /** 快照归属交易日 (FR-033 ②)。🚨 **实时独载基线 (source=realtime) 下它是「交易所的今天」** —— 库内一期快照都没有时, 屏上的数全部来自此刻, 归属正在进行的这一场 */
  asOf: string | null;
  ...
  /** 🚨 **OI 的归属交易日** (FR-033 ③) —— 与 asOf **不是同一天**。三个时点 MUST 各自成句, 🚫 MUST NOT 合并成一个「数据截至」。064 起实时档下它仍是那个归属日 (OI 盘中冻结)。🚨 实时独载基线下 OI 改由同一批实时给出, 而本字段取最近一个已收盘交易日 —— 两者由构造对齐 */
  oiAsOf: string | null;
  /** 这批数从哪来 (eod / premarket_backfill / realtime)。realtime = **实时独载基线**: 库内一期收盘快照都没有, 整条链由这一次实时取回撑起 (新锚盘中首访, 当晚收盘轮跑完即自愈) */
  source: string | null;
