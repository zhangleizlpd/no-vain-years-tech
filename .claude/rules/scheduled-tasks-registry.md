...
本地 launchd `.plist` 装在 `~/Library/LaunchAgents/`、cron 在 crontab —— 均在仓库外，本 rule 触发不到。新增这类任务时**手动**更新注册表（其仓库生成器 `scripts/jobs/<name>/setup.*` 能触发本 rule + CI 校验 LABEL，是主要兜底）。
