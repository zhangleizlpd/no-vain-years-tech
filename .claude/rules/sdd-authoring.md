...
- ❌ **规范性动词（`MUST` / `MUST NOT`）当强调号用** — 判据是**后果**不是位置：违反了会**静默出错**（采成另一天 / 写错字段 / DTE 差一天 / 判据被绕过而全绿）→ 顶 `MUST`；只是复述已有约束、或描述某分支「会怎样」→ 陈述句。同句里叠 `MUST … MUST NOT …` 多半是把一条约束说了两遍。**发多了的代价不是啰嗦，是真红线被淹掉**。⚠️ **别按 section 一刀切**（「Edge Cases 一律陈述句」是错的近似）：Edge Cases 里恰恰常驻「时区取错整体差一天」这类必须顶 `MUST` 的条目。降级前先问「这条的规范性有没有 FR 承接」——有则改成指针（`由 FR-0NN 在装配期拒掉`），没有则**别降，给它一个 FR 编号**（下条）。
