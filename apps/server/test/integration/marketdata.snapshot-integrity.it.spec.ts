 ...
 *
 * ═══ 时间锚 ═══
 * 谓词的「当日」= `(now() AT TIME ZONE 'America/New_York')::date`（session_date 是 us 业务日）。
 * 本文件用**生产同一个** `marketDateFor(['us'], now)` 求它 —— 谓词若误取 Asia/Shanghai，
 * 北京上午跑时两者差一天，所有场景当场红（这正是要它红的地方）。
 */
...
describe('期权快照逐合约完整性谓词 (Testcontainers PG, 与 check.sh 共享同一 .sql)', () => {
  ...
  beforeEach(async () => {
    ...
    contractIds.clear();
    today = marketDateFor(['us'], new Date());
  });
