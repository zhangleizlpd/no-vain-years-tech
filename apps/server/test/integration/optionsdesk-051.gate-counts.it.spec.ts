...
describe('051 T001 流动性排除计数按视角拆分 (Testcontainers PG, 逐条相等 + 重叠区不等式)', () => {
  ...
  async function seedBaseline(): Promise<Record<string, string>> {
    ...
    // ① 相对价差 14.00/13.00 ≈ 107.7% > 35% ⇒ 被挡; DTE 164 只在收租段 ⇒ **只**记收租。
    // 🚨 `bid` 蓄意压到 6.00（年化 11.7% < 收租 good 档界）—— 071 起 bid 年化达档的宽价差腿走
    // **机会支**进收租候选，那样这条腿就不再「被流动性门槛挡下」，本组计数的判据面会被换掉。
  ...
  /**
   * 重叠区那条腿 —— DTE 38 ∈ `[30,49]`, 有效成本 125 − 1.50 = 123.50 < 132.40 (过),
   * 相对价差 4.50/3.75 = 120% (被挡) ⇒ 期限段上它**同时**是建仓候选与收租候选。
   *
   * 🚨 `bid` 蓄意压到 1.50 (年化 11.7% < 收租 good 档界): 071 起 bid 年化达档的宽价差腿走
   * **机会支**进收租候选 ⇒ 收租那一侧的 +1 会变成 0, 而「两个视角各 +1」正是本条要钉的东西。
   */
