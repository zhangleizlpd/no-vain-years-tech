...
describe('052 检索层 (Testcontainers PG)', () => {
  ...
  async function seedMixedChain(): Promise<void> {
    ...
    const legs = [
      ...
      // 🚨 `bid` 蓄意压到 1.00（年化 < 收租 good 档界）—— 078 起 bid 年化达档的宽价差腿走
      // **机会支**进收租候选（078 FR-001），那样这条腿就不再「被流动性门槛挡下」，本组断言的
      // 判据面会被换掉；机会支自身的分支在 optionsdesk-078.wide-spread.it.spec.ts 逐条验。
