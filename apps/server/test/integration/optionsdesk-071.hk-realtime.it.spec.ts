...
describe('071 港股实时窄召回接线 (Testcontainers PG + Redis, 真 DI 容器)', () => {
  ...
  /**
   * T006b-① 的**深虚**腿 —— 两条都远在梯形窗之外, 用来把零 Δ 面那一支的候选窗钉住。
   * 定窗基准 spot = `104.25` (锚的盘中价)。
   *
   * 🚨 **077 起本组腿的判别面变了** (T002 翻绊线): 零 Δ 面的**收租**支不再走矩形宽窗, 改走
   * `rentBootstrapBudgetWindow` —— 行权价**无下界** (077 FR-003), 上界 = `axis × 1.03`
   * (`axis = min(spot 104.25, W 120) = 104.25` ⇒ `107.3775`)。⇒ 两条深虚腿**都**入窗。
   *
   * | 腿 | `K/spot` | 077 前 (矩形 `0.6×spot = 62.55`) | 077 后 (收租预算窗, 无下界) |
   * | --- | --- | --- | --- |
   * | `D-70` | 0.671 | 在窗内 | **在窗内** |
   * | `D-60` | 0.576 | 在窗**外** | **在窗内** |
   *
   * 📌 「港股取的是 `0.6` 那一项而非美股的 `0.7`」这条端到端判据随之**移交建仓侧** ——
   * 该比例表 077 起只服务建仓 (`leg-window.rules.ts` 的 `STRIKE_ENVELOPE_FLOOR_SPOT_RATIO_BY_MARKET`
   * 文档注释), 收租侧已无下界可判。接手的是
   * `optionsdesk-068.two-stage.it.spec.ts` 的 **077 臂①b**「hk 建仓 bootstrap 的矩形窗两侧都按
   * 港股取值」(同款两条深虚腿夹出 `0.6` 与 `0.7` 之间那条缝)。
   * 🚫 **不是**同文件的 077 臂① —— 那是**美股**建仓臂, 钉的是 `0.7`, 替不了 hk 这一半。
   * 🚨 两条的昨日 `|Δ|` 都低于收租带下界 `0.03` ⇒ 有昨日面时它们**本就不在梯形窗内**,
   *    这正是本臂差分的另一半 (同一份合约, 有面 ⇒ 不问; 无面 ⇒ 问) —— 077 后这条差分**更强**:
   *    零 Δ 面两条都问、有面两条都不问。
   */
  ...
  // ── T006b-① 新锚首日无昨日面 ⇒ 零 Δ 面兜底窗 (state_branch 8; 077 起收租侧换成预算窗,
  //    071 FR-002 那条 hk 0.6 落值的端到端判据随之移交建仓侧, 见 DEEP_OTM_LEGS 文档) ──

  it('🚨 T006b-① 无昨日 Δ 面 ⇒ 收租走预算窗, 深虚腿不再被行权价下界挡在外面 (077 FR-003)', async () => {
    ...
    // 🚨 **077 翻绊线**: 本臂此前断言 `D-70` 在窗内 ∧ `D-60` 在窗**外** —— 那是港股矩形下界
    //    `0.6 × 104.25 = 62.55` 的端到端判据。077 起收租支的行权价下界**整个消失**
    //    (FR-003: 「市价的固定比例」与收租判据毫无关系, 挡掉的是完全合格的腿且不产生任何信号)
    //    ⇒ 两条深虚腿**都**该被问到。
    expect(bootCodes).toContain('D-70');
    expect(bootCodes).toContain('D-60');
    // 上界仍在, 且按 W 轴而非市价固定比例: axis = min(104.25, W 120) = 104.25 ⇒ 107.3775,
    // 段内、界内的常规腿一条不落。
