 ...
 *
 * ⏳ **三个值都是占位, 标定在 053 T012** —— 🚫 **MUST NOT 当已标定值引用**。起手值没有分布
 * 依据: 唯一的取值理由是「量级上远小于 `K`, 且高于 dev 期多数链的候选数」, 与
 * {@link LIQUIDITY_TIER_BOUNDS} / {@link RATE_TIE_BAND} 那种带谷底与分位分析的定稿不是一回事。
 * 📌 占位期**取宽侧**是有方向的: 截断是减法, 宁可先不截, 也别在标定之前先把腿截没。
 *
 * 🚨 **分档不是三个对称的数** (FR-012): 全腿视角与两个意图视角的成员规模差一个量级, 一律取
 * 同一个数会让其中一档要么形同虚设、要么砍掉不该砍的。
 * 🚨 **全腿那一档有硬下界** (FR-014): `051` ship 的「点流动性排除数 → 切到全腿看被排除的腿」
 * 这个入口依赖那些腿仍在表内 ⇒ 标定它时,「被意图视角排除的腿在排序序列里的最深位置」是阈值的
 * 下界输入, **不能只看分布断点**。截在那些腿之前, 该入口就指向一张不含目标的表, **且不会红**。
 *
