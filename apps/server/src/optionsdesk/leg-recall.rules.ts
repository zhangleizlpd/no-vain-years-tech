 ...
 * **0 条**, 中位绝对价差 `$0.925` / 中位 bid `$1.90` —— 是真·宽报价而非 tick 假象。⇒ 本门槛拦
 * 的是该拦的, 无需再加「绝对价差逃生舱」那道复合判据。
 *
 * ⚠️ 该证伪的样本 spot ∈ [9.27, 178.25]。标的池扩到大量 `$2–$10` 的低价票时 tick 偏见会真的
 * 出现, 届时 MUST 重验这一条再决定要不要逃生舱。
 */
