 ...
 *
 * ## 🚨 当天到期、尚未收盘的腿被排除 —— 蓄意保留, 别当缺陷修 (2026-09-06 owner 裁定)
 *
 * **系统口径严于市场口径, 这是已知取舍。** 该腿在到期日当天**仍可交易** (EVIDENCE: 港股侧见
 * `specs/071-optionsdesk-hk-realtime-recall/spec.md` Edge Cases 「当天到期的腿在盘中仍可交易
 * (港股期权到期日当天交易至收盘)」), 而我方三个视角都不呈现它。owner 复核后**保留现状**。
 *
 * 🚨 **闸在读端, 不在本常量** —— 这是本段最容易找错的一处: 排除当天到期腿的**单点**是读端的
 * `expiryDate > marketDate` (`leg-retrieval.adapter.ts`, 047 FR-028a; 实时与离线两条路径各有
 * 一处、判据同形), 三个视角共用。本常量的 `min: 1` 只是把那条前置照进常量, 改它**不会**让当天
 * 到期腿出现。⇒ 真要改这条 policy, 动的是读端那道比较符, 且**两市同时生效**
 * (美股侧基准本就正确, 一直是全天隐藏; 港股侧自 #374 起与之同构)。
 *
 * 📌 全腿视角是唯一「只由读端把关」的视角 (它的 `dteBand` 恒 `null`, 见
 * {@link defaultCriteriaByTab}); 建仓/收租还各叠一道自己的下界, 但那是**策略段**不是可交易性判据。
 */
