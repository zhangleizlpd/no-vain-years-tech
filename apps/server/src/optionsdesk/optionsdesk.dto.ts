...
export class LegResponse {
  ...
  @ApiProperty({
    description:
      '078 宽价差机会标: 这条腿是从相对价差维度的**机会支**进来的 —— 市场很宽 ' +
...
export class LegTableResponse {
  ...
  @ApiProperty({
    description:
      '区块状态。三种「没有表可看」蓄意不折叠: chain_not_ready (会有的, 还没采到) / ' +
      'no_listed_options (该标的在交易所根本没有挂牌期权, 终态、别等) / read_failed (跨 ctx 读故障)',
...
export class ChainReportResponse {
  ...
  @ApiProperty({
    description:
      '屏级状态。四值与选约表同源, 三种「没有表可看」蓄意不折叠: chain_not_ready (会有的, 还没采到) / ' +
      'no_listed_options (该标的在交易所根本没有挂牌期权, 终态、别等) / read_failed (跨 ctx 读故障)',
