 ...
 * 默认数据集只镜像真实导出**形态** (表头 27/13/11 列 + 汇总聚合行 + 资金行/XD 行),
 * 标的 / 数量 / 金额 / 日期全部纯合成；合成标的标识单源于 `FIXTURE_INSTRUMENTS`
 * (`synthetic-holdings.ts` 同源复用)。变体 (缺 sheet / `--` / 未知类别) 通过 options 组合。
 *
...
/**
 * 合成标的标识 (纯虚构, 非真实上市证券; dev 库 instrument 表无此代码)。
 * 本 builder 默认数据集与 `synthetic-holdings.ts` 共用的单一来源。
 */
export const FIXTURE_INSTRUMENTS = {
  main: { code: 'ZQX', name: '合成甲股份' },
  second: { code: 'ZQY', name: '合成乙科技' },
  repo: { code: 'ZQR', name: '合成逆回购' },
} as const;

const M = FIXTURE_INSTRUMENTS.main;
const S = FIXTURE_INSTRUMENTS.second;

/** 默认持仓 2 行 (纯合成; 次行含 `--` 与空列, 镜像真实脏度)。 */
export const FIXTURE_HOLDING_ROWS: CellValue[][] = [
  // prettier-ignore
  [M.code, M.name, '19600', '300', '0.0145', '', '', '', '', '770', '0.0411',
    '2345.6', '0.1319', '300', '600', '770', '0.3', '1400', '8', '0.0145',
    '14', '13.45', '', '0.04', '-0.02', '0.08', '0.12'],
  // prettier-ignore
  [S.code, S.name, '49000', '-700', '-0.0141', '', '', '', '', '-1750', '-0.0333',
    '--', '--', '-700', '-1750', '-1750', '0.7', '7000', '4', '-0.0141',
    '7', '7.25', '0.0527', '-0.09', '-0.04', '-0.0936', '-0.14'],
];
...
export const FIXTURE_CLOSED_ROWS: CellValue[][] = [
  // prettier-ignore
  ['2025-12-18', M.code, M.name, '2018.12', '0.0941', '0.0353', '0.0588',
    '10.4', '11.35', '-0.1265', '93', '16.88', '2025-09-16'],
];

/** 默认流水 4 行：资金转入 (其他) / 买入 / 除权除息 (XD 前缀) / 卖出。 */
export const FIXTURE_TRADE_ROWS: CellValue[][] = [
  ['2025-09-12', '', '', '', '其他', '0', '0', '60000'],
  // prettier-ignore
  ['2025-09-16', '10:21:09', M.code, M.name, '买入', '1700', '10.4',
    '-17685.3', '17680', '5.3', ''],
  // prettier-ignore
  ['2025-11-06', '15:30:00', M.code, `XD${M.name}`, '除权除息', '0', '10.9',
    '420', '420', '0', ''],
  // prettier-ignore
  ['2025-12-18', '13:48:36', M.code, M.name, '卖出', '1700', '11.35',
    '19283.42', '19295', '11.58', ''],
];
