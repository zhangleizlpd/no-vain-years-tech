 ...
 *
 * EVIDENCE: 富途 `get_option_chain` 不提供历史交易日的链 —— 官方文档明文「此接口不支持查询
 * 已过期的期权链，**结束日期** 参数请输入今天或未来的日期」; 且 `start` / `end` 的原文说明是
 * 「该日期指**到期日**」⇒ 接口**没有观测日入参**, 结构上问不出「某个过去交易日当时的链是什么
 * 样子」。出处 https://openapi.futunn.com/futu-api-doc/quote/get-option-chain.html
 * (2026-09-03 复核)。本模块多处复述此结论, **指针以此处为准**。
 *
 * ## 失败语义显式 (T014 的承重设计)
 *
