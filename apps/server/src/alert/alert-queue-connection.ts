...
export class AlertQueueRedisLifecycle implements OnApplicationShutdown {
   ...
   * 跑完**（官方 API 原文 "waits for current jobs to finalize"；不等待要显式传 `force`）。
   * Nest 对同模块 providers 的 `onModuleDestroy`
   ...
   *      https://docs.bullmq.io/api/classes/v5.Worker.html（close 等 in-flight）
   *      https://docs.nestjs.com/fundamentals/lifecycle-events（段间屏障；⚠️ 该页**没有**
   *      记载同模块内 providers 的串并行 —— 那一条的出处是 `@nestjs/core` 源码,
   *      见 `marketdata-queue-connection.ts` 的 EVIDENCE）
