 ...
 * 他在隧道内打的是代理, 代理才持有它。
 *
 * **一把 token 守五条 location**: 研报投递 / 锚待审提交 / **锚直写** / **标的枚举** /
 * **标的批量基础信息**。
 *
 * ⚠️ 后两条是**读**。此前这把 token 只能写 —— 现在它还能读整张标的注册表 (上市公司公开
 * 元数据, 敏感度低, 但爆炸半径确实变了)。按下面那条「分法按权限层、不按端点数」的判据,
 * 这是本 token 第一次跨到第二个权限层, 值得单独记一笔; 仍判定不值得为读面另开一把 ——
 * 它与写面共命的分析原封不动成立 (同一 SOPS blob / 同一台 guest 机 / 同一份渲染出来的
 * nginx conf), 而读面的边际损失远小于写面。
 *
