 ...
 * 行 = 重复投递，查不到 = 取号争用。**不用 `meta.target`** —— 重查法与该字段的形态完全无关。
 *
 * ASSUMED: 「`meta.target` 在 PG 连接器下常返回约束名而非列名数组，且随 Prisma 版本 /
 * adapter 变形」—— **未验证**, 本仓无实测。它成不成立都不改变本处该怎么写: 选重查法的理由
 * 是「不依赖那个字段的形态」, 而不是「已确证它不可靠」。⇒ 别拿它去论证别处该不该读
 * `meta.target`。
 *
 * ## 元数据回声：一律取自 `row`
 *
