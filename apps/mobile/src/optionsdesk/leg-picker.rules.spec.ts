...
function table(overrides: Partial<LegTableResponse> = {}): LegTableResponse {
  return {
    symbol: 'us:PEP',
    // 069 契约增量: 每 K 行军判决 (门控 = 收租视角; 071 T007 起两市一律) —— mock 基线取缺省 null。
    march: null,
...
describe('🚨 FR-017 —— 未选水位是常驻分支：显式提示，MUST NOT 静默假设', () => {
  ...
  it('🚨 051 FR-020 —— 水位未选 + 收租视角 ⇒ 追加的就地注明说的是「成员集合不变、只是零推荐标」', () => {
    ...
    // 🚨 **2026-09-01 扫描前先把标名挖掉**：`fitBadge` 已改成「Δ带内」（标名自带 Δ）⇒ 下一行
    //    那条 `toContain` 引用的标名会自己触发这条防线。挖掉标名后判据一字未松：正文里再出现
    //    Δ / 0.05 / 0.40 照样当场红，旧文案「展示全部 Δ 档（0.05–0.40Δ）」回潮仍拦得住。
    expect(note.replaceAll(COPY.fitBadge, '')).not.toMatch(/Δ|0\.05|0\.40/);
