...
export const OPTIONSDESK_COPY = {
  ...
  legPicker: {
    ...
    /**
     * 🚨 未选水位时收租腿 Tab 的**就地注明**（plan D-SOT-4）：Δ 档取三档并集而非静默取一档。
     * 上下界与 server `leg-tab.rules.ts` 的 `RENT_DEPTH_UNION_BAND` 同源（该常量从三档表派生）
     * —— 跨 ctx 拿不到它，改档表时**须回改这一行**。判据在 server，这里只负责把它说出来。
     */
