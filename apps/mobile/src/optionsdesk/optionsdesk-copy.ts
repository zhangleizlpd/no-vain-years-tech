...
export const OPTIONSDESK_COPY = {
   ...
   * 🚨 **`chainNotReady` 与 `readFailed` 蓄意分开**：前者是「这张表还没有内容」这一事实、
   *    要说明何时会有；后者是跨 ctx 读故障。合并成一句「加载失败」= 把事实说成故障。
   * 🚨 前者**不等于**「采集还没轮到」—— 它还盖着「该标的根本没有挂牌期权」这一支，那支永远
   *    不会有内容。措辞判据见 `chainNotReady` 上方（#361）。
   */
  legPicker: {
    /**
     ...
     *
     * 🚨 **本状态有两种成因，文案对两者都必须为真**（2026-09-05 订正，跟踪 issue #361）。
     *    server 侧两条语义不同的 `return null` 落到同一个 `chain_not_ready`
     *    （`leg-retrieval.adapter.ts` 的「库内无合约」与「有合约无快照」），而前者里混着一类
     *    **永远不会有内容**的锚 —— 该标的在交易所根本没有挂牌期权。原文案「该标的的链正在补
     *    ……补完这张表就有内容」对这一类是**假承诺**：没有补数在跑，也永远补不出内容。
     *    EVIDENCE: prod 上 6/140 个锚属这一类（hk:00291 / 00371 / 00777 / 00960 / 06117 /
     ...
     *    🚫 在 server 把两种成因分成两个 state 之前（#361），本文案 MUST NOT 再出现
     *    「补完就有内容」这类**只对其中一支成立**的承诺。
     */
    chainNotReady:
