...
export const OPTIONSDESK_COPY = {
   ...
   * 🚨 **三种「没有表可看」两两蓄意分开**（#361 起 server 侧就分开了，这里逐值对齐）：
   *    `chainNotReady`（会有的，还没采到 —— 要说明何时会有）/ `noListedOptions`（交易所根本
   *    没挂，永远不会有 —— 要说明别等）/ `readFailed`（跨 ctx 读故障 —— 给重试）。
   *    合并任意两支都会让呈现层对其中一支撒谎（#362 修过一次那样的假承诺）。
   */
  legPicker: {
    /**
     ...
     *    已拆成 {@link noListedOptions}，server 的 `LegTableResponse.state` 是两个值。
     *
     * 🚨 **但仍 MUST NOT 写「补完这张表就有内容」**（#362 修掉的那句）。收窄之后它依然不成立：
     *    链发现**还没为这只锚跑完**时，成因判不出来，一律落在本值上 —— 其中就混着「跑完才会
     *    发现它没有挂牌期权」的那些。⇒ 说清机制（在补、无需操作）可以，给结果保证不行。
     *    落法是把两种出路都说出来，于是两支都为真。
     *    EVIDENCE: prod 上 6/140 个锚没有挂牌期权（hk:00291 / 00371 / 00777 / 00960 / 06117 /
     *    06186；us 侧 0 个）—— HKEX《The List of Stock Option Classes Available for
     *    Trading》(2026-09-01, 154 classes) 按正股代码与中英名双角度均无这 6 只，而库内有
     *    合约的 22 只 hk 锚在同一份名单里 22/22 命中（正对照，证明「查不到」不是解析口径问题）。
     */
    chainNotReady:
      '期权链数据未就绪 —— 该标的的链正在补，新建的锚会自动触发一次补数，无需操作。补完之后这里要么出表，要么明确告诉你这只标的没有挂牌期权。',
    /**
     * #361：**终态，非错误，别等**。与 {@link chainNotReady} 的差别不是程度而是方向。
     *
     * 🚨 **MUST 说出「锚本身照常有用」** —— 否则用户会以为这只锚建错了。锚同时承载估值 V 与
     *    雷达档位，期权表只是它的一块；没有挂牌期权不影响另外那些。
     * 🚫 **MUST NOT 出现「稍后」「重试」「正在」这类时间词** —— 它们把终态又说回过程态，
     *    正是本值要消灭的东西。
     */
    noListedOptions:
     ...
     *
     * 🚨 **MUST NOT 读作「建议买入 / 该选它」** —— 服务端的判定**完全不看视角成员**，故存在
     *    「带标却进不了任何意图视角」的腿（050 实测约占期限段合格腿五分之一）。措辞是**唯一**的
     *    消歧手段：它说的只是「这条腿的 Δ 落在你当前意图的带内」这一个事实。
     * 🚨 **措辞取纯事实陈述**：任何带「合适 / 该选」语气的词都会把这枚标读回「建议买入」，而它
     *    只报告一个可验证的事实。`layeredRanker` 的排序首键读的**就是**这同一个判定（带内的腿
     *    整体在前）⇒ 措辞与序同源，屏上「为什么它排前面」当场自洽。
     * 🚨 主体是**认得出来的汉字**，不是几何符号 —— 纯符号连「这是什么」都要查图例（mockup
     *    阶段实证：月度链标初版画成空心方块，spec 作者本人评审时仍需发问）。`Δ` 只作限定词，
     *    且同屏那一列的列头副标本就是「Δ 带判据」（{@link columnSubDelta}），读者已见过该符号。
     * 🚨 **与 068「带外」标不是一对反义词**（易读错，此处钉一句）：那条说的是**预测落带**外
     *    （`bandStatus`），本条说的是 **Δ 推荐带**内 —— 两个不同的「带」，可同现于一行。
     */
    fitBadge: 'Δ带内',
    monthlyBadge: '月',
     ...
     * 🚨 **MUST NOT 带市场名**（071 起）：064 写这句时读路径只有美股，港股锚当时是「未支持
     *    市场」、走的是降级支，**到不了这里**；071 把港股接进实时档后其 `realtimeDegrade`
     *    恒 `null` ⇒ 港股收盘后**同走本支**。写死「美股」于是对着港股锚说美股的事 —— 且港股
     *    收盘时美股常常正开着，那句话**对两个市场都不成立**。市场信息由页签与标的代号承担，
     *    不缺这一份；本条只说「此刻不是常规交易时段」这一件与市场无关的事。
     */
     ...
     * 📌 吃 `badge` 而不内嵌标名二字 —— 与 {@link marksFollowIntentNote} 同一范式，
  ...
  chainReport: {
     ...
     *
     * 🚨 **四句两两不同、🚫 不合并成一句「暂不可用」** —— 未就绪（等就有）/ 没有挂牌期权
     *    （别等，#361）/ 无现价（行轴无从定义）/ 读失败（可重试）对用户是四件完全不同的事，
     *    合并之后每一种的处置都说不清。
    ...
    degraded: {
      ...
      // #361: 与上一支**同形不同因**。🚫 MUST NOT 合并 —— 一个该等、一个该走。
      noListedOptions: {
        title: '该标的没有挂牌期权',
        text: '交易所没有为它挂出期权合约 —— 与「链还没采到」「有链但全被门槛挡下」都是两回事。',
      },
      noSpot: {
        title: '标的现价暂不可用',
   ...
   */
  march: {
    /** 收租行推荐章（FR-016）—— 单字，载体同「Δ 带内」标。 */
    recommendBadge: '荐',
    /**
     * 四类劣档灰显微标（凹 #2 / 陈 #3 / 并 #4 / 叉 #1，只标不删）。
     *
     * 🚨 **#1 用「叉」不用「异」**（070 FR-004）：本段语气纪律把「异常」留给真故障，而交叉报价
     * 是**报价几何**的事实陈述（买 ≥ 卖），措辞与弹层 #1 那句「报价交叉」同源 —— 屏上两处对得上账。
     * 📌 #1 天然只在离线档出现：实时口径下交叉腿在召回层就被剔出候选（070 T001），根本不成行。
     */
    inferiorMarks: {
      concave: '凹',
      stale: '陈',
    ...
    /**
     * 070 口径行（FR-003）：这一梯的判决建立在哪一天的收盘上 —— 弹层题头**一次说清**，
     * 故 13 类逐条目 MUST NOT 再各加「昨收」尾缀（FR-004；逐条加只会把同一句话说 13 遍）。
     * 实时档不渲本行：那一批就是此刻的盘口，写「基于…收盘」是反向伪装。
     */
    basisEodClose: (sessionDay: string) => `基于 ${sessionDay} 收盘`,
    /**
     * 070 θ 模式被动标示（FR-009）：模式由 server 配置定，UI 不提供切换入口，这里只如实标示
     * 「这一次是按什么判的」。默认 φ 模式**不渲本行** —— 默认态加标示等于给每个用户永久噪音。
     * 🚨 θ 下 φ 读数行同时收起（判据是年化 argmax 不是再投资线，两个一起渲 = 静默混用两模式）。
     */
    modeThetaReadout: '选档判据 · 自身年化最大',
    /**
