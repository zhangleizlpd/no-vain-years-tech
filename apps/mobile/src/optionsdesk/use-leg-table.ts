...
//    同一份派生结果的三种**客户端过滤**（切 Tab 不重新请求）。分三次请求会让三个 Tab 的
...
export function useLegTable(symbol: string): UseLegTableResult {
  ...
  // 🚨 全量腿 —— 这里**不 slice、不排序**（两者都是 server 的职责）；唯一的筛是按 `tabs` 分 Tab。
  ...
  const sections = useMemo(() => legPickerSections(legs, tab), [legs, tab]);
