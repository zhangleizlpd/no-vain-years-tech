...
--
-- ⚠️ **代价：非交易日跑会假红**（当日本就无快照）。⇒ timer 排在 us 交易日的采集窗之后，本谓词
--    **不自己判日历**（判了就又是一次「拿邻近物证明自己健康」）。周末多推一条的成本 < 引入循环
--    信任的成本。
--
-- ═══ 当日 = **ET 的今天**，不是宿主的今天 ═══
...
-- ─────────────────────────────────────────────────────────────────────────────────────────────
WITH bounds AS (
  SELECT (now() AT TIME ZONE 'America/New_York')::date AS current_day
),
...
SELECT
  (EXISTS (SELECT 1 FROM verdict WHERE degraded))::int AS exit_code,
  CASE WHEN EXISTS (SELECT 1 FROM verdict WHERE degraded)
       THEN '🔴 期权快照逐合约覆盖率跌破阈值'
       ELSE '✅ 期权快照逐合约完整性达标' END
  || ' (当日 ' || (SELECT current_day::text FROM bounds)
