#!/usr/bin/env bash
#
# marketdata-calendar-health.sh — 交易日历健康探针（044 US3 / FR-010 / FR-014）。**零逻辑胶水，判断不在这里。**
#
...
# （`apps/server/test/integration/marketdata.calendar-044.health.it.spec.ts`：埋 25h/27h 心跳
# + 主源/降级 servedBy 断言）。本脚本只剩三步：**跑谓词 → 打印 summary → exit 谓词给的码**。
#
#   ⇒ 🚨 **本文件里不得出现任何 if / 阈值 / 比较 / 市场清单 / 「主源是谁」**。
...
#
# 判据（**全在谓词里**，此处仅复述供人读，非本脚本行为）：cn / hk / us 任一 `last_success_at`
# 超 26h（或缺行）→ 不健康；或任一 `served_by` 非**该市场的**主源（cn/hk=tencent、us=futu）→
# 降级亦不健康（FR-014「降级 ≠ 健康」）。us 于 2026-07-31 随换源纳入监控面（044 原「us 有意
# 排除」的理由 = 无备源可用，已被 `us: [富途 L1, 腾讯 L2]` 两活源推翻）。
#
