...
--
-- ═══ 🚫 临时排查 findings 时别 cast 成 text 去匹配中文 ═══
--
-- 本文件读 findings 走 JSONB 操作符(`jsonb_typeof` / `->>`), 是对的 —— 这一段是防**人工或
-- agent 临时查库**时顺手写出另一种形态: `findings::text LIKE '%未知股票%'` **恒不匹配**。
-- JSONB 把非 ASCII 存成 `\uXXXX` 转义, `::text` 出来仍是转义串(`未知股票`),
-- 中文字面量永远撞不上, 而且**不报错、只是恒 false**。
--
-- 2026-09-04 我(本条注释的作者)实撞: 查「哪几轮有死码 502」, 上述写法对 09-02/09-03/09-04
-- 三行全返 `f` —— 其中一行(id 970)我几分钟前刚用 `left(findings::text, 1200)` 亲眼看到过那条
-- 错误。改判 ASCII 侧(`LIKE '%vendor_error%'`)后三行全中, 结论从「只有 09-04 有」翻成「三天都有」。
--
-- 🚨 这类假阴性**看起来像好消息**(「没有毒批」「干净对照日」), 且与真阴性在输出上不可区分 ——
--    正是宪法「下结论前先问: 如果反例存在, 我的管道能看到吗」要拦的形态。
--    ⇒ 临时查一律走 JSONB(`findings @> '[{"kind":"failure"}]'`), 要文本匹配就匹 ASCII 侧的
--    key 或值, 别匹中文。
--
-- ═══ digest 的形状与取舍 ═══
--
...
--    (`same_day_retry` / `premarket_backfill`, #261 续) 自己开 SyncRun 行之后, 美股期权
--    每天 08:00 / 18:00 都会有一轮排在夜链之后。
--    📌 073 之前这里举的例子是「hk 期权每晚 23:40」—— 港股那两个补救触发点已随 073 退役
--       (港股补漏改由轮 2 `hk_option_oi_settle` 承担, 而它是**正经 tick 轮**、不是按需轮,
--       故不构成本条的反例)。判据与排序一个字没变, 只是例子换成美股那两档。
--
