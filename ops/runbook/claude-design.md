...
| **wrap / zero-size** | 定宽盒塞不下文本                 | 按**垂直区间重叠**给 rect 分组，组数 > 1 = 折行；元素零尺寸    |
...
🚨 **判据 2026-08-14 订正 —— 原写法 `getClientRects().length > 1` 会大量误报**（`053` 抽屉 mockup 实撞 **33 处假阳性 / 真值 0**）：

- **8 处**是 `align-items: center` 下**不等高 inline 子元素**造成的「同一行多 rect」—— 元素实测高 16px，一行，但 rect 有两个。
- **25 处**是**本该换行的散文**（注记、说明段）—— 它们换行是设计，不是塞不下。

⇒ **正确判据 = 按垂直区间重叠给 rect 分组，组数 > 1 才算折行**；并且**只对定宽/单行盒扫**，散文段落先排除。

📌 **为什么这条 drift 危险**：假阳性不是「多报几个无害警告」——**33 比 0** 的信噪比会让人直接把整条探针当噪声关掉，而它是唯一能抓到「缩略图尺度看不出的折行」的那道防线（正是上一段那个代价换来的）。**探针误报到不可信 = 探针不存在。**
