...
① 文中关于 **vendor / 交易所 / 第三方平台运行时行为**的断言，此刻能指出出处吗？
   • 能 → 带上指针（一手 URL / fixture / spec 路径 / ADR / FR / 具名实验 / 实测日期+**观测值**）
     观测值只限**外部公开数据**；私有账户 / 持仓 / 成交 / 订单的数字不写，改定性表述 + 出处（谁 / 哪天 / 哪份私有证据）
...
② **复述仓内已有结论时，指针要跟着走** —— 本仓两条独立证据链都在最后一跳掉了指针
   ...
   做不到就点名来源方（「某某 2026-09-01 直查 prod」），**不写成无主语的「实测」**。
③ **别升格证据等级** —— 「单日探针观测到 X」复述成「X 是交易所规格」，是本仓实撞过的形态。
④ 不用自评置信度代替出处（「大概 / 应该是 / 比较确定」）。二元：给得出出处，或直说未验证。
...
② **怎么写**：
   ...
   **观测值本身就是合法出处**（`72/100`、`→ 403`）——比只给日期更强，且是本仓主流写法。
   仅限**外部公开数据**（vendor / 交易所 / 公开行情）；私有账户 / 持仓 / 成交数据的观测值不适用 —— 写定性表述 + 出处，数字留 docs/private/evidence/。

③ **复述别人的实测要点名是谁测的** —— 不写成无主语的「实测」，读者会以为你复算过。
④ **不用自评置信度代替出处**（「大概 / 应该是 / 比较确定」）。二元：给得出出处，或直说未验证。
