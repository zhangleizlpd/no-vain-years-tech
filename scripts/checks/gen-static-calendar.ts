  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i + 1; j < lines.length && j <= i + 5; j++) {
      ...
      for (const day of days) {
        ...
        if (statuses.get(day.text) === CLOSED_STATUS) continue;
        trading[market].add(`${ctx}-${pad2(Number(day.text))}`);
      }
    }
  }
...
//
// 🚨 **不含 us** (044 clarify Q3: prod 实证无 \`{us}\`-only 维度 + gate 取 OR ⇒ us 日历不阻塞
// 任何同步)。覆盖假设与其绊线见 \`static-calendar.adapter.ts\` 注释。
