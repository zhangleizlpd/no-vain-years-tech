...
11. **两个滤除计数不是可选装饰**（FR-008）——权利金门槛作用于全 Tab ⇒ 有腿真的从 UI 上消失，**部分推翻 047 FR-005**。计数是这笔取舍的唯一补偿。
...
- [ ] T003 [P] [Server] **扩展 `check-optionsdesk-rule-constants.ts` 覆盖召回阈值**（FR-007, SC-009, plan D-RECALL-2）：给既有脚本加第二条不变量「两道门槛阈值 + 三段 DTE 界的字面量只许出现在 `leg-recall.rules.ts`」，**被禁字面量从该源文件自身派生**（照抄既有那条的做法 —— 在检查器里写死等于自己就是第二处硬编码）。🚫 **MUST NOT 新写检查脚本** —— 同类不变量分两个文件，调参时必漏一个 → verify: 🚨 **先证明它会红** —— 故意把某个阈值字面量抄进 `get-legs.usecase.ts`，跑检查器**必须 exit≠0 且指出该行**；改回后 exit 0。红不了说明这条守门等于没装（同源纪律：049 T005「先对旧实现验红」）

- [ ] T004 [Server] **use case 接召回 + 两个滤除计数**（FR-005, FR-006, FR-008, FR-010, plan D-RECALL-1/D-API）：`get-legs.usecase.ts` 的 `deriveLegs` 把 `legTabs(...)` 换成 `recallTabs(...)`；权利金门槛在**进入腿列表之前**滤（全 Tab 消失），流动性门槛在**Tab 归属时**滤（只 build/rent）；累计 `filteredOut.premiumFloor` / `filteredOut.liquidityGate` 两个计数并挂上 `LegTableView`。047 三条读端过滤（仅认沽 / 仅标准 / 到期日 `>` 当日）**原样不动**（`FR-010`）→ verify: `get-legs.usecase.spec.ts`（Small，既有 stub prisma）新增用例 —— 被权利金门槛滤掉的腿**不在** `legs[]` 里且计数 +1；被流动性门槛滤掉的腿**在** `legs[]` 里但 `tabs` 只剩 `all`；两个计数互不串台

- [ ] T005 [Server-IT] **召回集合 IT**（US1 全 4 条 + US2 全 5 条 AS, SC-001/SC-003）：新建 `apps/server/test/integration/optionsdesk-050.recall.it.spec.ts`（Testcontainers 真 PG，形制照 `optionsdesk-047.leg-picker.it.spec.ts`）→ verify: 🚨 **断言必须是「成员集合逐条相等」不是 `length > 0`** —— 召回换代的失败形态是「返回了腿、数量也合理、只是成员错了」。逐条：① 建仓集内**零条**有效成本 ≥ spot（`SC-001`）② `DTE ∈ [30,49]` 的腿同时出现在两个 Tab ③ `DTE = 400` 只在 `all` ④ greeks 缺失的腿**在**意图 Tab 里（047 下会被挡掉）⑤ 一分钱腿三个 Tab 全不见 + 计数含它（`SC-003`）⑥ 宽价差腿不在 build/rent、**在** `all` ⑦ 无 `bid` 与无 `ask` 各一条 ⑧ 某 Tab 被清空 → 返空集合且**不是 404**，面板照常有数据
