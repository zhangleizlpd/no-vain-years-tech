...
- [X] T021 [Verify] **真机验收**（`SC-001`, `SC-003`, `state_branch` 16/17）：Mate50 dev-client。① **一屏可见 / 零纵向滚动**（`SC-001`，🚨 用真机读数不用 web 那组，Guardrail 12）—— 🚨 **本条本轮风险升高，必测不得跳**：spec 把页脚从 2 计数改成**3 计数 + 一行 hint**（约 +16px），而一屏余量至今**只在 web 上验过**（mockup overflow = 0）。真机窄约 13% ⇒ 那 16px 可能正好吃掉余量。越线时按 `FR-041` 的顺序**先压曲线、再压页头**，🚫 网格不得纵向滚；② **横滑到最右端**末列完整露出、不越界不回弹（`state_branch` 16）；③ **长按 vs 横滑的手势归属**与手感（`state_branch` 17）；④ **色阶五档在真机屏上可分辨**（`SC-003`）；⑤ 段外列淡出与「被门槛挡下」在真机上**不查图例可分**（Guardrail 7 的真机复核）。→ verify: 逐条读数写回 spec § 真机验收。🚨 **这是开 PR 前的最后一道闸**，撞到 FAIL 即停下修，MUST NOT 记为「已知问题」往下走<br><br>✅ **已完成**（Mate50 `CET-AL00` dev-client，标的 `us:ACN` = 本机最大链 1708 腿 / 17 到期日；逐条读数写回 spec `## 真机验收`）。①②③④⑤ 全部通过，其中 **⑤ 首轮 FAIL 并当场修掉**。验证面：mobile 全套 **1695 绿** · runtime-smoke **214 passed** · lint 0 error · typecheck 绿。<br>🚨 **⑤ 首轮 FAIL = 列头整行在真机上高度塌成 0**（到期日 / DTE / 月度 chip / `段外` chip 全灭）。根因：列头行为拿 `onTrackLayout` 多包了一层 `<View className="flex-1">`（column 方向），把 `LegColumnPane` 自带的 `flex-1` 从**宽轴翻到高轴** ⇒ `flexBasis:0` 塌高、再被自身 `overflow-hidden` 裁净；下面的范围框行因为 `LegColumnPane` 直挂 `flex-row` 而始终正常，两行并排就是现成的对照。修法 = 给那层包装加 `flex-row`。<br>🚨 **这条 web e2e 结构上照不到** —— `chain-report-out-of-band-chip` 的可见性 + `toHaveCount(2)` 断言在 web 上**一直是绿的**：CSS 下 auto 高的 flex 容器仍按 max-content 参与父高计算，Yoga 直接得 0。**同一份断言在两个平台上问的不是同一个问题。**<br>📐 **① 的前置粗算基准是错的，已由真机订正**：设备实为 `base=1088x2400 @462dpi` ⇒ **376.8 × 831.2 dp**、应用区 **793.8 dp**，不是起草期按的 `360×780`（`wm density` 只报 `Physical density: 520`，本机开了 override，**有效值不在它的输出里**）。真机实测：恢复列头行（+107px / 37.1dp）之后，网格底到读法行之间仍有 **108.4 dp 可让出余量**，`FR-041` 的压缩顺序**一处都没动用**，曲线与页头原样。<br>📐 **零纵向滚动 = 逐像素证**：页头区与空白区各上滑一次，前后截图 `mean row delta = 0.000`。<br>📐 **③ 手势三情形各自实测**：直接快拖 → 列移动且 4/4 帧无读数面板；按住 4s → 读数面板出现且网格区零像素变化；按住后拖 254px → 读数由 `09-04` 换到 `09-11` 而指示条 thumb 恒在 `x=208`。<br>🚧 **一个显式未验项（故意留）**：列头四个元素验到三个，`月` chip 未验 —— 它在**任何环境下都产不出来**（日历源是「指数当日有 bar = 开市」⇒ 表里结构上无未来交易日，而月度判据要求候选日在册；ACN 16 列里 11 列是第三个周五，实测 0/16 被标）。缺陷早于 055 且与 047 选约表共用同一份查询 ⇒ 归 **issue #45**，🚫 本片不代偿。<br>⚠️ **取证管道自身踩过三个假象，写进 spec 防复踩**：`adb shell "input swipe … &"` 子进程随 shell 退出被杀；`input motionevent` 分次注入不形成同一条触摸流（会得到假的「十字线不跟手」）；`input tap` 后立刻 screencap 会抢在重渲染之前（连出两次假的「点击没生效」）。<br>📐 **① 的前置算数（web 侧量高，🚫 不当验收）**：在 `360×780` 与 `390×844` 两个视口下逐块实测，**导航栏之下的内容需要 625 px** = 页头 94 + 切换 29 + 两处间隙 16 + 曲线 74 + 网格 317 + 读法行 13 + 页脚 82。两个视口下 `document.scrollHeight === clientHeight`（web 侧零纵向滚动）。<br>　　⚠️ **这组数只降低风险、不构成结论** —— 049 实测 web 与真机差约 13%，plan 明写「余量判定 MUST 用真机那组」。粗算 Mate50（1080×2340 @3x ⇒ 360×780 dp）扣状态栏 ~24 + 手势条 ~24 + Stack 头 ~56 ⇒ 可用 ~676 dp vs 需要 625 dp，**余量约 51 dp**；页脚改三计数 + hint 那 +16px 吃掉的正是这一段。<br>　　🔧 **越线时按 `FR-041` 的顺序动**：先压**曲线**（当前 74 px，是六块里唯一可无损压缩的）→ 再压**页头**（94 px）。🚫 网格（317 px = 8 行 × 32 + 列头/范围框）MUST NOT 纵向滚，也 MUST NOT 削行。<br>📋 **真机上要看的五件事**（逐条写回 spec § 真机验收）：① 打开报表**零纵向滚动**、页脚三计数与那句 hint 都在屏内；② 横滑到最右端**末列完整露出**、不越界不回弹；③ **直接拖 = 移列 / 按住 300ms 再拖 = 十字线**，两者不打架、手感不粘；④ 色阶**五档**在真机屏上两两可分（`brand-50/100/300/500/700`）；⑤ 段外列淡出与格内 `╱`「被门槛挡下」**不查图例**就能分开。<br>🚀 **怎么起**：`/run-local-env` 拉起全栈（dev server + Metro + Mate50 dev-client），报表入口在**标的详情屏**「链分析 · 机会分布」那一行（046 三块之后、选约区块之前）；也可深链 `/(app)/optionsdesk/chain-report/us%3AACN`。⚠️ dev 库 `2026-08-13` 那期 12 条链数据齐备，直接看得到满网格。
