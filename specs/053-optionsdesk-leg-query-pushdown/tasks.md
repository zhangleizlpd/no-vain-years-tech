...
- [ ] T016 [Mobile] **修本片自引入的 React #301 白屏**（`FR-020`–`FR-022` 的回归；**2026-08-14 对照实验定性后新增**）：`use-leg-table.ts` 的 render 期条件 `setState` 在「选水位 → 意图改判」这条路径上**不收敛**，被 ErrorBoundary 接住 ⇒ **真机上表现为选完水位偶发白屏**。<br>🚨 **定性靠对照实验，不是推测**（同法同机，两边同一条 `-g "US3-AS3" --repeat-each=5`）：
