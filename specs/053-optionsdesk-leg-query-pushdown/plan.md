...
| 位置 | 内容 | 数据来源 |
| --- | --- | --- |
| sticky 区块头 | 当前条件下符合条件的总数 | `matchedCount` |
...
⏳ **`memberCount`（未覆盖条件下的候选数）本 plan 暂不立** —— `052` 把六维条件并入召回层后它不再是免费的（要用默认条件再召回一遍）。归 `/speckit-clarify` 待澄清项 1；定了再回填本节与 `D-ORDER-1`。
...
列改版后 sticky 栈占屏比 ≤ 35% 与横滑到最右端末列完整露出（`SC-003`，基线 `051` 实测 138.5dp / 27.9%）· 视角切换与预热的**手感** · ⏳ `052` 遗留三项（抽屉盖不盖得住 Tab 栏 / 输入法 / ⓘ 热区）—— 补验还是显式排除归 clarify 待澄清项 3。
...
3. **摘掉 `placeholderData: keepPreviousData`** —— 换 key 那一拍同步 setState 打成死循环，整屏被 error boundary 接住，且**它不会自行收敛**（`D-ASYNC-1`）。
...
12. **拿 `052` 的六维边际计数加总充当 `memberCount`** —— 它们是 per-维度的，被多维同时挡下的腿会双计（`D-API-1` 的 ⏳ 项）。
...
| 2 | `[Server]` | 截断纯函数 + 分档常量（带 `⏳` 占位）+ 可注入阈值 + `matchedCount` |
...
| 13 | `[Verify]` | 真机验收：列改版几何 / 切换手感 / ⏳ `052` 遗留三项 → 写回 spec |
