...
- mobile：屏体/hooks/helpers `apps/mobile/src/portfolio/`；薄路由 `apps/mobile/app/(app)/portfolio/holdings.tsx` + `apps/mobile/app/(app)/portfolio/trades/[symbol].tsx`（`parseSymbol` 复用 014 canonical `cn:ZQX`；2026-09-14 amend：示例代码改为合成代码，原因：公开仓不收录个人金融业务数据，见 information-boundary）；工具栏 `apps/mobile/src/portfolio/watchlist-main-screen.tsx`
...
- [X] T006 [P] [US3] [Server] **EP3 标的流水 UC + controller**：`list-trades.usecase.ts`（等值 (accountId, market, code) 查 trade_record，`ORDER BY tradeDate DESC, tradeTime DESC NULLS LAST`）+ query DTO 校验（market/code 必填 → 缺失 400）+ controller GET `portfolio/trades` + **Testcontainers 单测**：样本标的全量 9 条时序正确（2026-09-14 amend：示例标的改为定性表述，原因同上） / 未交易标的空 items 200 / 资金行（code null）天然不命中
