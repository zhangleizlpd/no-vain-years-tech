...
- [X] T007 [Gate] **行军参数在港股的适用性判定（单向否决，不是标定）**（FR-013, FR-014; plan §D5; state_branches 14; US1）：复用 069 回放脚本（`docs/private/evidence/069-replay-calibration.ts`，local-only）改造入口喂 hk 收盘链，跑**三条判据**：① 形状类条件在港股净链上的触发率与美股**同量级**（不显著偏高——偏高 = 把常态噪音当异常，表现为推荐档莫名其妙地短且不报错）② 流动性下限不致港股收租候选**整梯清零**（清零梯占比不高于美股基线）③ 档界参数货币无关、直接沿用。🚨 **「同量级 / 不显著」是人工看表裁决，不是自动门** —— impl 期把三张对照表呈给 owner，由其判过不过，🚫 禁自行拍一个倍数当阈值。**三条全过**才放开 `get-legs.usecase.ts:705` 门控为 `perspective === 'rent'`，并把 `optionsdesk-070.offline-ladder.it.spec.ts:405-414` 臂④ 翻成正例；**任一不过**则门控不动、结论落 spec 并起后续标定片 → verify: 三判据各出一张对照表落 spec 新段 + owner 裁决记录；🚨 本条是**判定**不是标定 —— 发现要调 φ/β/γ/`OI_MIN` 任一取值 ⇒ 停下报 user（Guardrail 8）；放开时门控改动须有 IT 臂（hk 收租 march 非 null）+ 070 臂④ 同步翻面 —— 📌 **于 2026-09-05 落，owner 裁定三条全过 ⇒ 放开门控**。三张对照表 + 裁决记录 + 四条射程限制落 spec 新段「行军参数适用性判定」；回放脚本 `docs/private/evidence/071-t007-march-applicability.ts`（**没沿用 069 那份**：它按单 session 全库合一统计，本条要的是**分市场**触发率 + 竞争性风险的分母控制，另写一份反而比改它干净）。落地四处：① `get-legs.usecase.ts` `marchBlock` 去掉 `market` 维（连带删形参与调用点实参，无 orphan）② 070 臂④ 翻正例 —— 断言取「判决面与 us 逐值相同」而非「非 null」（两市夹具逐字段同构 ⇒ 同一管道必给同一份判决），**DTE 蓄意不进对拍**：两市「今天」差一天（请求时刻 08-11 12:00 ET = 08-12 00:00 HKT，`daysToExpiry` 按市场取参 #263）⇒ hk 恒少 1，单独钉一条 ③ `071-hk-realtime.contract.ts` `assertMarchGate` 绊线**换方向**（现钉「港股有判决块」，谁把 market 维加回来当场红）④ 本表第 14 行翻面。🚨 **未另起 071 IT 实时臂**：`marchBlock` 门控不看档位（该函数注释即判据）⇒ 实时 / 离线走同一段代码，070 臂④ 已是「hk 收租 march 非 null」的 usecase 层 IT 臂；这是**结构论证不是实证**，已在 spec 射程 ② 写明。变异留档（各一次、均只打红目标臂）：把 `market` 维加回门控 → 070 臂④ 红（`expected null not to be null`，其余 9 臂全绿）· 同一变异 → 契约 `assertMarchGate` 第三条红。🚫 φ/β/γ/`RENT_MARCH_OI_MIN` 一个取值都没动
...
- [X] T009 [Gate] **SC 收口 + ADR-0068 回写 + PR 门**（SC-001, SC-004, SC-005, SC-006, SC-007; FR-010 落档, FR-018 收口; plan §D3/§D4/§D6; US1/US2/US3）：① **SC-001** = T006a-① 臂留档；② **SC-004**（美股逐值相同零例外）= T005-②③ 臂留档；③ **SC-005** = T006a-③ 对拍臂留档；④ **SC-006**（每条状态分支有断言且能红）= T006a/T006b 变异清单留档；⑤ **SC-007** = T008-① 留档；⑥ **ADR-0068 回写** —— sunset trigger #1 标注消费（裁决 = **不升格降级态、值域不扩**，理由：两者对用户是同一件事，而扩值域要连带动契约值域、四条穷举文案与前端穷举映射）+「后果·仍并存」段的 ② hk 锚收租按 T007 结论更新；⑦ spec frontmatter `status → implemented` + `updated_at` bump → verify: `git fetch origin && pnpm nx affected -t lint typecheck test build --base=origin/main --skip-nx-cache` exit 0 + gate 脚本（server-moat / test-size / optionsdesk-rule-constants / time-semantics / identifier-boundary / repo-layout）全 0；🚨 跑门前必 `git fetch`（066 因此报过一次假全绿） —— 📌 **于 2026-09-05 落**。①–⑤ 汇成 spec 新段「**SC 收口**」一张表（SC → 证据落点 → 形态），🚫 蓄意不写 task 号而写**判据形态** ——「有 task 覆盖」不等于「有断言且能红」；同段附 `state_branches` 覆盖说明（14 条里 13 条有 IT，第 14 条自 T007 起由 070 臂④ + 契约 `assertMarchGate` 承载，EC8 蓄意零覆盖的理由一并写明）与 FR-010 值域不扩落档。⑥ **ADR-0068 两处回写**：sunset trigger #1 标 ✅ 已消费（裁决 = 不升格降级态、值域不扩，理由两条落 ADR）+「后果·中性」段的 ② `hk 锚收租` 从「仍并存」划掉并写明它是**单向适用性判定**而非标定片。⑦ **`status` → `implemented`**（user 2026-09-05 裁定）—— 判据是「`status` 语义 = impl 全完，而 T010 是 Ops 验收不是 impl」；SC 收口表里 SC-002 / SC-003 已如实标 ⏳ 并指向 issue #314，验收状态另有载体，不被 `status` 掩盖。🚨 **顺带核实纠正一处**：#314 的 09-04 评论记「选约表 roster 查询不过滤 `withdrawnAt`（`leg-retrieval.adapter.ts` 两处），修复方案待 owner 裁决」—— 该项**已于同日由 #345 消费**（`git log -S` 实证：两处 `withdrawnAt: null` 由 `1837b728` 引入，且采集侧对账同批下沉到合约级、052 靶场加 H-WITHDRAWN、071 加实时臂并做过定向变异）⇒ **不另开 issue**，改在 #314 补闭环记录，🚫 免得造一个重复项

- [X] T010 [Ops] **部署后真时段验收（合并后勾，带到期日）**（SC-002, SC-003; plan §D6; state_branches 1/2; US1/US2）：prod 部署后，连续 **3 个港股交易日**在港股连续竞价时段各抽查 ≥1 次 —— 断言 ① 港股锚选约表无降级横幅、档位条呈秒级时点（SC-002）② 午休时段请求落中性收盘档、零告警、零对外呼（SC-003）。🚨 **本条 MUST 带到期日并挂 issue** —— 同类 task 在 066 因为没有到期日而至今未勾、把那份 spec 永久卡在 `implementing`，本片 MUST NOT 重蹈：开 task 时同步建 issue 并写明「部署后第 3 个港股交易日为到期日，逾期未勾则在 issue 里记录阻塞原因」→ verify: 三次抽查各留一条记录（日期 + 时刻 + 截图或响应片段）回填本条；issue 号回填本行 —— 📌 **跟踪 issue #314**，**到期日 2026-09-03**（部署后第 3 个港股交易日；08-31 部署当日港股已收盘 ⇒ 首个可抽查日为 09-01） —— 📌 **2026-09-13 口径订正 + 回填**（owner 裁定，同步落 #314）：① **判据由「连续 3 个港股交易日」改为「累计 3 次、各落不同港股交易日」** —— 09-04 / 09-07 取样后 09-08…09-11 四个交易日全部断档，「连续」按字面已补不回（除非作废重跑三天）；相邻日期从未承载独立判别力，🚫 已完成的两次 PASS 不作废。② **已完成 2/3**：**1/3** 2026-09-04 10:36 HKT —— `hk:09988` 收租视角 `● 实时 10:36:51`（秒级、与请求同秒）、无降级横幅，Mate50 真机 · release 0.16.0 · prod；证据 `docs/private/evidence/071-t010-spotcheck-1-2026-09-04-1036.png`。**2/3** 2026-09-07 10:20 HKT（周一）—— `● 实时 10:20:50`、零降级横幅（蓝色常态条非红色告警条）、35 行 bid/ask 齐全，release 0.20.0 · prod v0.49.0；**顺带复核 T005 周一效应未复现**（若未生效，周一的坏法是一张零外部信号的正常收盘档表，而现场落实时秒级档）；证据 `…071-t010-spotcheck-2-2026-09-07-{1019-radar-hk,1020-rent-realtime,1021-allLegs-eod}.png`。③ **SC-003 已单独收口**（2026-09-07 12:05 HKT）：午休落灰色中性收盘档，`勿据此下单` / `降级` / `不可用` / `未就绪` 全表各 0 命中；**零外呼是直接观测** —— 判据取 `[068] window-size`（打在外呼之前），午休那次请求到了而该行缺席，并以 11:51 连续竞价那次为**正控制**（该行在）；证据 `071-sc003-377-{pre,passive,post}-2026-09-07-*`。⇒ **本条只余 SC-002 第 3 次**。④ **新到期日 2026-09-14**（下一个港股交易日，已核在 `static-calendar.data.ts` hk 表内且不在 `STATIC_CALENDAR_HALF_DAYS.hk`）。🚨 **该次 MUST 同时记建仓视角** —— 09-04 那次建仓因 vendor 活列死码回落收盘档（外因，根因 #345 于 09-04 22:20 起在 prod 写软下架）、09-07 未记 ⇒ 建仓视角至今无干净样本，而 SC-002 说的是「选约表」、不限视角；若仍降级，说明还有第三种形态，比抽查本身更值得追。🚫 收租视角那两条断言（零降级横幅 / 秒级时点）已验两次、逐字同款，第 3 次在收租上**判别力增量为零**，值钱的是建仓那一格 —— 📌 **2026-09-14 抽查 3/3 完成 ⇒ 本条收口**：14:02–14:05 HKT（下午连续竞价段；时刻按手机钟，与 prod 响应 `Date` 头相差 ≤1s（同一刻读数 14:06:15 vs 14:06:16），取证机本地钟慢 30s 已校正），`hk:09988`，Mate50 真机，prod = main `d9ca7962`（release server 0.51.1，#410）。⚠️ 与前两次的差异如实记：本次客户端是 **dev-client 加载本地 JS 打 prod**，非 release 包 —— 071 前端零代码改动（plan §D4），判据面是 server 下发档位的渲染，不受影响。① **建仓视角首个干净样本**：`● 实时 14:02:59`、25 行 bid/ask 齐全、零降级横幅；UI dump 对 `勿据此下单` / `降级` / `不可用` / `未就绪` / `未取到实时` / 行内「收」标全屏各 0 命中；14:05:12 点「刷新」⇒ 档位条翻 `● 实时 14:05:13`（**与请求同秒**），前后两帧盘口逐腿跳动（如 105P 09-29 `3.15/3.25 → 3.17/3.30`）⇒ 活盘口非缓存 ⇒ 「若仍降级说明还有第三种形态」这一支**未触发**，09-04 那种建仓回落收盘档本次未复现。② 收租视角 `● 实时 14:02:59`、38 行、零降级横幅（同款第三次，仅记录）。③ 全腿视角灰色 `收盘档 09-11 · 非常规交易时段`，符合 T006a-⑦（全腿恒回落收盘档），作对照。证据 `docs/private/evidence/071-t010-spotcheck-3-2026-09-14-{1403-allLegs-eod,1404-rent-realtime,1404-build-realtime,1405-build-realtime-refreshed,1405-build-no-autorefetch}.png` + 三份同名 `.uidump.xml`。⇒ SC-002 3/3、SC-003 已于 09-07 收口 ⇒ **T010 勾选**，#314 同步关闭
...
| 13 | 全腿视角回落收盘档 | T006a-⑦ |
| 14 | ~~港股收租默认不出阶梯~~ ⇒ **T007 判定后与美股同门控**、不出阶梯仍是显式双 `null` | T007（070 臂④ + 契约 `assertMarchGate`） |
