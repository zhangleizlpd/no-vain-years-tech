...
| 层                  | 条数                                             | → FR / US 覆盖                                     | 零覆盖登记 |
| ------------------- | ------------------------------------------------ | -------------------------------------------------- | ---------- |
| `state_branches`    | 20（+2，clarify Q3 / Q4 各派生 1）               | 全部落 FR-001–FR-023                               | 无         |
| Clarifications      | 5（配额用满）                                    | 每条各自落 FR / SC / Out of Scope                  | 无         |
| Acceptance Scenario | 21（US1 ×4 · US2 ×6 · US3 ×5 · US4 ×4 · US5 ×2） | 全部落 FR                                          | 无         |
| Edge Case           | 7（+1，clarify Q4 派生）                         | 6 条落 FR；1 条（月度日非交易日前移）**故意零 FR** | ✅ 见下    |
| FR                  | 27（+4：FR-007a / 010a / 011a / 014a）           | 全部落 SC 或 US                                    | 无         |
...
- **clarify 已完成（5 轮，配额用满）**，`status` 已转 `clarified`。五个决策点：计数与说明落位 / 两个标落位 / 计数是否可操作 / 带标却不在意图 Tab 的腿 / 腿数变多要不要解释。
- 本片范畴来自主 plan §2.3（已按 `049` / `050` 实际交付重划），非凭 P2 行原表述——原表述只写了四项，实际七块。
- 下一步：`/speckit-plan`。clarify 阶段最担心的那个决策点（计数与就地说明在不进常驻区的前提下放哪）已定案，且顺带发现既有就地说明本就在常驻区内 ⇒ 本片会让常驻区高度**下降**而非持平。
