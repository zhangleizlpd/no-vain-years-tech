...
>
> 📌 **2026-09-13 状态复核（判决不变，只改机器可读形态）**：这两条原先仍写作 `- [ ] **DEFERRED…**`，而 `/sdd-auto-impl`（`.claude/commands/sdd-auto-impl.md:32`）、`/speckit-implement`（`.claude/skills/speckit-implement/SKILL.md:64-66`）、`/speckit-tasks-verify`（`.claude/commands/speckit-tasks-verify.md:12`）**一律只按 `- [ ] T<N>` 判 pending，读不到行内的 DEFERRED 字样** ⇒ 会被反复派单（已实际发生一次）。⇒ 两条的 checkbox 改为 `- 🚫`，本 spec 的 `- [ ]` 计数归零。🚨 **重开 US2 不是回来勾这两条**：判据走 [`spec.md`](./spec.md) Assumptions「美股侧错开重开的判据」，形状走 [`plan.md`](./plan.md) §B'，届时**新开一片**、写新 task。
...
| ~~真 Redis 时序 IT（新建）~~ 🚫 DEFERRED —— 写过、跑过、**判红**，未入库；09-06 已归档，源码树内**不存在**该文件 | `docs/private/plans/2026-09/09-05-075-us2-stagger-redis-it-deferred.ts`（local-only；原路径 `apps/server/src/marketdata/sync-flow-stagger.it.spec.ts` 已不在盘） |
...
- 🚫 **DEFERRED（2026-09-05）** T006 [Server-IT] **真 Redis 时序：delay 从 child 完成时刻起算 + 失败传播不被延后**（FR-014, FR-016, FR-022; SC-004, SC-005; plan §B/§C; state_branches 11/12; US2）：新建 `sync-flow-stagger.it.spec.ts`，自起 `RedisContainer('redis:7-alpine')`（**只要 Redis、不挂共享 PG**，testing.md §4 步 3；先例 `alert/push-dispatch.processor.it.spec.ts:2`），用 `FlowProducer` 装一亲一子、child 跑满一段可分辨的时长 → verify: 三臂 ① `parent.processedOn − child.finishedOn ≥ delayMs`（SC-004，sb 11）② **对照臂** delay=0 ⇒ 同一差值为个位数毫秒（两臂缺一不算实证，testing.md §7）③ child 失败 ⇒ parent 立即被唤醒、时延与未加错开时同量级（SC-005，sb 12，FR-016）；📌 **FR-022 无需新增日志**：`sync_run` 已有逐维度的 `started_at` / `finished_at`（`schema.prisma:1374/1375`），下游起跑 − 上游完成事后直接可查 —— 本 task 只需在文件头把这条查法写下来；执行走 `pnpm nx test server <file>`（🚫 禁 `vitest --root`，找不到 schema）
  ...
  > 📌 承载这次取证的 spec 文件**未入库** —— 它的臂 ③ 断言的是被证伪的那条性质，原样进仓即是一条恒红的测试。⇒ 09-06 整份归档到 `docs/private/plans/2026-09/09-05-075-us2-stagger-redis-it-deferred.ts`（local-only，文件头写明它为什么在那、臂 ③ 为什么是故意红的、以及拷回源码树的重跑命令）；源码树下的 `apps/server/src/marketdata/sync-flow-stagger.it.spec.ts` **已不存在**。重开时只取夹具，🚫 MUST NOT 照抄被测形状（[`plan.md`](./plan.md) §B'）。

- [X] T007 [Gate] **零回归全量门**（SC-006; plan §C; US1）：不写新断言，跑**权威 PR 门口径**的全量 —— `nx affected -t lint typecheck test build runtime-smoke --base=origin/main --skip-nx-cache`（该命令串的唯一权威是 [local-verification §2 命令矩阵](../../docs/conventions/local-verification.md) 的「PR 门」行；🚫 **MUST NOT** 换成 `run-many -t typecheck,lint,test` 之类的窄口径 —— 少了 `build` / `runtime-smoke` 就是拿弱证据去勾 PR 的 hard-gate checkbox）→ verify: 全绿，且**结果判定按 local-verification §3「exit code 会说谎」**（别只看退出码）；且**零个既有断言被改写或删除** —— 逐文件取证 `git diff --stat` 对 `option-anomaly.rules.spec.ts` / `sync-option-snapshot.usecase.spec.ts` / `sync-flow-assembler.spec.ts` / `dimension-executor.spec.ts` 的**删除行**逐条归属（签名改动引起的调用行更新可接受，断言体改动不可接受）；🚨 Testing Invariant：「判据语义一条没改」由**既有测试全绿**承担，MUST NOT 新写一条「语义没变」的断言
  > ✅ **2026-09-06 实跑读数**（`pnpm nx affected -t lint typecheck test build runtime-smoke --base=origin/main --skip-nx-cache`；`origin/main` = `68df26fc`、merge-base = `bfe1157d`；affected = server / api-client / mobile）。判定按 local-verification §3 **grep 终态串**而非 exit code：终态 = `Successfully ran targets lint, typecheck, test, build, runtime-smoke for 3 projects`，全程零 `Failed tasks` / 零 `No test files found`。逐 target：**lint** 0 errors（267 + 579 warnings，仓内无 `--max-warnings` ⇒ 不是门）· **typecheck** 零 `error TS` · **test** server 502 files passed | 13 skipped、5890 tests passed | 87 skipped；mobile 121 files / 1888 tests passed · **build** server 绿（api-client `generate` 一并跑） · **runtime-smoke** server 六步 boot smoke 走完 + mobile `expo export` → playwright `274 passed`。
  > 🔍 **「零既有断言被改写或删除」的逐文件取证**（`git diff --numstat origin/main...HEAD`，**三点**）：`option-anomaly.rules.spec.ts` = **499 added / 0 deleted**（Guardrail 3 的机器判据满分）· `sync-option-snapshot.usecase.spec.ts` = **151 added / 1 deleted**，那唯一一条删除是 **import 行**（补 `anomalyDteExchange` 具名导入），**非断言体** ⇒ 判可接受 · `sync-flow-assembler.spec.ts` / `dimension-executor.spec.ts` **压根不在 diff 内**（零改动，印证 T005 / T006 已整块回退）。⚠️ 查询语义要认准：**两点** `origin/main..HEAD` 会多出一条 `db-option-chain-discovery.adapter.spec.ts` 0/170 —— 那是 main 侧 `3ca1e478` / `232636f4` 的改动被反向计入，不是本分支删的，故取证一律用三点。
  > 📌 **三条遗留项的显式交代**（前面 task 报上来的，不是新发现）：① `sync-option-snapshot.usecase.spec.ts:789` 新增一条 `max-lines-per-function` **warning**（213 行 > 150）—— **判：接受，不拆 describe**。三条理由：同文件 `:247` 早有同类（603 行）、全仓共 190 条同类，规则是 `warn` 且仓内无 `--max-warnings` ⇒ lint exit 0，机器上不是门；拆了也只从 2 条降到 1 条（`:247` 照旧超），收益边际；**最要命的是拆 describe 会在本 task 正在取证的那个文件里制造删除行，把上面那份「零断言改写」的证据毁掉** —— 门不该改自己的证据。要收另开 `refactor`，且必须连 `:247` 一起收。② 生产导出 `anomalyDteExchange`：全仓 3 处引用 = 声明（`sync-option-snapshot.usecase.ts:179`）+ **唯一**生产调用点（同文件 `:455`，累加器懒创建）+ 测试；`:175-177` 注释已写明「导出只为可测、双闸下经公开入口不可达、🚫 MUST NOT 在别处调」⇒ **无误用**。③ 累加器 `state` 字段：`rg '\.state\b' apps/server/src/marketdata/` 只命中 `option-anomaly.rules.spec.ts`（11 处）与闭包自身的局部变量，**零生产读取**；`option-anomaly.rules.ts:274` 注释已把「生产代码 MUST NOT 读它、更 MUST NOT 写它」钉死。本门是这条纪律的唯一检查点，已查。

- [X] T008 [Ops] **上线后观测口径落到复盘批 4**（SC-007, SC-008; plan §Gate 0.1; US1/US2）：把两条 post-merge 判据写进 `docs/private/plans/2026-09/09-05-prod-memory-hang-postmortem.md` §九 批 4 的清单 —— ① 连续 7 个采集窗无被中断轮次 + 容器内存事件计数与重启次数保持 0（SC-007）② 任一采集窗内宿主可用内存最低点 ≥ 200 MB（SC-008，该阈值取事故当晚崩溃点 80 MB 的 2.5 倍，是**保守边界不是标定值**）→ verify: 批 4 清单可见这两条且各自写明数据来源（采样 CSV 的哪几列）；📌 **SC-007 / SC-008 是本片唯二不进 PR 门的 SC，这是故意的** —— 它们要真实采集窗才能求值，写在这里是为了不让它们变成「只有口号、没有验证手段」的那类 SC（sdd-authoring 反模式 ②）；⚠️ 同时登记一条批 2 的连带陷阱：09-08 那轮曲线量到的峰值是**改动后**的，拿它反推容器内存上限与堆上限的终值会系统性偏小
  > ✅ **2026-09-06 已落**（复盘 §九 批 4 新增「075 的 post-merge 验收」小节 + 「批 4 连带登记两条」；文件 local-only ⇒ 本 commit 只含 tasks.md）。两条 SC 各自点名了采样 CSV 的列（表头见 `docs/private/evidence/memhang-memsample.sh:65-68`）：**SC-007** 三处数据源 —— ①「无被中断的轮次」不在 CSV 里，查 `marketdata.sync_run` 的 7 个业务日 ②「内存事件计数」= 列 `nvy-tight-app-1_oom` / `nvy-tight-app-1_oom_kill` 全窗恒 0（⚠️ cgroup 累计，别跨容器重建做差）③「重启次数」不在 CSV 里，走 `docker inspect` 的 `RestartCount`；**SC-008** = 列 `mem_avail_kb` 按 `ts` 切窗取最小值 ≥ 204800，辅证列 `psi_some_avg10` / `pgmajfault`，并写明 200 MB 是**保守边界不是标定值** + 🚫 MUST NOT 事后调松。⚠️ 顺带记了一条前置：采样器只排到 09-08 08:00 CST，**要凑满 7 个窗得先续上**，否则 SC-007 无从求值。
  > 🆕 **本次决策带来的变化也一并登记**（比原计划多的那条）：错开已 deferred ⇒ **09-08 那轮变成「带 US1、不带错开」，恰好是判断错开要不要开、开多少的干净基线，且只有这一次机会**（开了就再也量不到「开之前、US1 之后」）。⇒ 批 2 取数 MUST 把「上游 `option_contract` 完成后内存回落到稳态所需时长」**单独列出**（数据源 = 列 `nvy-tight-app-1_mem_bytes` 的时间序列，起点取 `sync_run` 的 `finished_at`）—— 它正是 [`spec.md`](./spec.md) Assumptions「美股侧错开重开的判据」的**第一条**。
