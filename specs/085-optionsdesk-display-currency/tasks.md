...
| FX 真 vendor 探针（新） | `apps/server/test/integration/optionsdesk-085.fx.vendor.spec.ts`（整文件 `RUN_FX_VENDOR_IT` 门控、默认 skip ⇒ Large 档后缀，per `testing.md` §2.1） |
| 共享 vendor 传输（改） | `apps/server/src/marketdata/vendor-http-client.ts`（T002 加字节读口 `requestBytes` + `FetchResponseLike.arrayBuffer?`；GBK 通路必需，既有 JSON / 文本两口逐字不变） |
