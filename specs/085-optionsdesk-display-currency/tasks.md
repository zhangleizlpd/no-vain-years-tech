...
2. **金额一律 `Prisma.Decimal`，出边界一律 string**；🚫 Number 中转。
...
4. **`displayCurrency` MUST NOT 进任何 `where`** —— 它只影响呈现，不得成为第二条数据路径（账号隔离仍在既有查询条件里）。
5. **取数时刻走 ingestion time**（`capturedAt`，绝对时刻，ADR-0066 第二条轴）⇒ 出 ISO UTC 由 mobile 按设备本地展示；optionsdesk 侧 🚫 裸用 `Intl.DateTimeFormat`（`check-time-semantics` Rule B 当场红）。
...
7. **mobile 🚫 `Intl.NumberFormat`**（`format/compact-amount.ts` 文件头硬纪律，Hermes 上 Intl 支持不确定）；币种一律**三字母代码**，🚫 货币符号。
8. **mobile 不自算汇率或折算** —— 折算全在 server（`check-optionsdesk-rule-constants` 不变量 #8 已在扫客户端自算默认值）。
9. **跨 ctx 不可 import**：FX 解析器 🚫 复用 `alert/realtime-quote.rules.ts`（股票 88 字段 vs FX 22 字段，且 eslint `boundaries` 拦，ADR-0053）；拒绝壳 🚫 import `marketdata/refusing-collection.adapter.ts`。两处均**照写法另落一份**，并存是已知状态。
...
- [ ] T011 [Mobile-E2E] **e2e 扩臂：三臂币种状态 + 切档重算 + 降级 + 汇率行**（FR-001, FR-005, FR-006, FR-007, FR-009, FR-010, FR-011, FR-013; plan D7, D8; state_branches 1, 2, 3, 4, 5, 14, 15, 16, 17, 18, 19, 20, 21; US1/US2/US3）：`e2e/optionsdesk-trading-account-positions.spec.ts` 扩臂（数据全合成，mock `/me` + refresh + 本片端点；🚨 e2e 必 mock refresh，per `reference_mobile_test_pitfalls`） → verify: `pnpm nx run mobile:e2e-public` 全绿，臂：① 进入 hk 页签 ⇒ 展示 HKD、无汇率行（branch 2, 14）② 点选择器 ⇒ 展开三档、当前档带勾（branch 16）③ 选 CNY ⇒ 收起、金额即时重算、分组与排序按新币种重排、汇率行出现（branch 17, 15）④ 🚨 **币种状态三臂（缺任一臂，错误实现都会绿）**：**③a** 切档后**进持仓详情再返回** ⇒ **仍为所选币种**（列表屏未卸载；照 083 T015③b `:652` 形态）· **③b** 切档后**返回雷达再进入** ⇒ **复原为该市场原币种**（本屏卸载；照 083 T015③ `:616` 形态）· **③c** 在 hk 切 CNY 后**切到 us** ⇒ **us 为 USD**，再切回 hk ⇒ **仍为 CNY**（branch 3, 4, 5；缺 ③a 或 ③b，store 实现与屏级 `useState` 实现都会绿；缺 ③c，单格状态与每页签一格都会绿 —— FR-005 / SC-005）⑤ 汇率不可用响应 ⇒ 降级行带原币种标、屏上**无任何 CNY 单位的港股金额**（branch 8；SC-003）⑥ 降级组沉底且两列标「合计不完整」（branch 11, 13）⑦ 汇率加载中 ⇒ 加载态、无未折算数字跳变（branch 18）⑧ 空持仓账号 ⇒ **选择器仍可见可点**（branch 20）⑨ 切档后**进持仓详情** ⇒ 详情页金额与价格**仍为原币种**（branch 21；FR-010）⑩ 切档后列表**无二次跳变**（SC-004）⑪ 切档前后列宽与字号断言不变（SC-007）；定向变异：a. 状态改放 `trading-account-store.ts` → ④③b 红 · b. 状态改单格 → ④③c 红 · c. 详情页也接展示币种 → ⑨ 红（留档）
...
- **T011 是 mobile 侧汇合点**：三臂状态断言需要 T009 的状态形状 + T010 的呈现。
- **T013 在 T011 之后**：e2e 全绿再上真机，避免把 e2e 能抓的问题留到真机。

## state_branches 覆盖预检（analyze 期逐条 grep 的基准）

> 🚨 **本表编号 = `spec.md` frontmatter `state_branches` 的行序（`spec.md:18-38`），MUST 逐行同序**。

| # | branch（摘要） | 落点 |
|---|---|---|
| 1 | 进入持仓页 ∧ us ⇒ USD，不折算，不出汇率信息 | T006-①② + T008-①② + T011-①（hk 同构臂）+ T012 |
| 2 | 进入持仓页 ∧ hk ⇒ HKD，同上 | T006-①③ + T008-①② + T011-① |
| 3 | 切换市场页签 ⇒ 该页签自己的当前值 | T008-④ + T011-④③c |
| 4 | 在某页签切币种 ⇒ 仅该页签生效 | T008-③ + T011-④③c |
| 5 | 离开交易账户页再进入 ⇒ 复原（不持久化） | T011-④③b |
...
| FR-005 不持久化、本次停留保持、两页签各自独立 | T008-③④ + T011-④③a③b③c |
...
| SC-005 再进入两页签各为原币种 100%、停留内保持 100%、跨页签影响 0 次 | T008-③④ + T011-④③a③b③c |
...
| US1-AS5 CNY 下进详情 / 订单页 ⇒ 仍原币种 | T011-⑨ |
| US2-AS1 进 hk 页签 ⇒ HKD、与上线前数值排序完全一致 | T006-①③ + T011-① |
| US2-AS2 币种 = 原币种 ⇒ 无汇率信息、无折算数字 | T006-②③ + T010-① |
| US2-AS3 切 CNY 后离开再进入 ⇒ 复原 | T011-④③b |
| US2-AS4 切 CNY 后进详情返回 ⇒ 仍 CNY | T011-④③a |
| US2-AS5 us（USD）切到 hk（本次未切过）⇒ HKD | T008-④ + T011-④③c |
| US2-AS6 hk 切 CNY 后切 us ⇒ USD；再切回 hk ⇒ 仍 CNY | T008-③ + T011-④③c |
...
MVP = **T001 → T007**：到这里 server 侧折算链与契约完整成立，US1 的「金额按选定币种折算、价格类不折、降级不骗人」在 IT 层可独立验收（`SC-006` 的逐字节比对也在此闭环）。US2（T008-T009）补 mobile 选择器与每页签一格状态这条 FR-005 / SC-005 的正确性底线，US3（T010）补降级呈现。T011 汇合 e2e 三臂，T012 并行补契约缝，T013 真机窄屏门，T014 收口发 PR。

Clear 检查点批次：`T001-T003` / `T004-T005` / `T006-T007` / `T008-T009` / `T010-T011` / `T012-T014`（每批次后停顿提醒 `/clear`，per Constitution §III）。
