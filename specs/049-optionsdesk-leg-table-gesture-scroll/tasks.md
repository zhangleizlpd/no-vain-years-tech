...
- [X] T007 新增栈高回归断言：纵滚到腿区后表头仍可见，且数据区可见 **≥3 行**腿数据；覆盖「未选水位 → 同屏两条就地注明」的最坏档 (FR-011, SC-006, plan D-TAB-4)
  → verify: 在窄视口 config 下跑；若现有 config 视口过高导致断言恒真，**标注「真机验收补」并挪进 T008**，🚫 MUST NOT 凑一条恒真断言充数
  → 📌 **落地判决（2026-08-11 实测，390×844 即本文件既有窄视口）**：栈顶钉在 `y=64`、sticky 栈净高 **185px**、栈底至视口底剩 **595px = 12.4 行**（@48px）⇒ 要让「数据区 < 3 行」成立，栈净高需 **> 636px**（= 844 − 64 − 144），是预算的 **3.4 倍** ⇒ 「≥3 行」**恒真、零鉴别力**。对照实证：把 Tab 行加厚 32px（`py-sm`→`py-lg`）后栈高断言当场红（217 > 194），而同一轮「可见行数」仍有 **11.7 行**、「≥3 行」照样绿。⇒ **「数据区 ≥3 行」标「真机验收补」挪进 T008 M8**；本 task 落地的是**有牙的那条**——直接量 sticky 栈净高 ≤ 预算（182 + 12px 度量余量），外加「纵滚后表头仍钉在顶」与两条前提自检（最坏档真的到达 / 真的滚进腿区）

## 收尾

- [X] T008 真机验收单 M1–M8 逐条（Mate50 dev-client + 数值探针）(SC-001 – SC-006, plan D-TEST-3)
  → 📌 **验收结论与全部实测读数已回写 `spec.md` § 真机验收记录（T008 · 2026-08-11）**。M1–M7 全绿；M8 栈高绿（161dp / 11.2 行 / 30.6%），a11y 的**角色播报未验**（本机无 TalkBack，仅华为屏幕朗读，经 owner 确认不测）——已在 spec 的「未验项与原因」段写明，未按通过计。旋转档**不适用**（app 锁竖屏），视口变化路径由密度变更等价覆盖。`SC-001` 阈值措辞与本探针口径不可直接对比，已在 spec 标注建议 P1 订正
...
| SC-006 数据区 ≥3 行（含最坏档） | T007（**栈净高 ≤ 预算**，行数判据 web 侧恒真已实测）· **T008(M8)**（行数判据本体） |
