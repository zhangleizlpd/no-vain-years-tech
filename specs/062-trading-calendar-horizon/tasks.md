...
以下六项**故意不做**，不是遗漏，下次 analyze 不要当缺口补 task：
...
6. **交易日历「区间聚合」问法的收编** —— T012 的门禁把豁免定在**问法的形状**上：`date: { gt/gte/lt/lte }` 区间聚合（「这段区间里有几个交易日」）与写操作豁免，其余未识别形状一律 fail-closed。因此 `alert/evaluate-alerts.usecase.ts` 那处算估值 staleness 的 `tradingDay.count({ where: { date: { gt, lte } } })` **不在门禁射程内**，本片**故意不收编**：三态判据回答的是「某天是不是交易日」，对区间计数无意义，强行 import 只会造出一个 unused import；且该窗口是**历史区间**（快照日 → 触发日），由既有活源链填充，coverage 空洞风险远低于本片修的「今天」那一类。
   ⚠️ **但它不是零风险**：历史段真出现空洞时该 count 会少算 ⇒ staleness 偏乐观 ⇒ gate 偏放行，那仍是同一个病（closed-world）的变体。**若将来要收编**，做法是给 `TRADING_CALENDAR_PORT` 增一个区间计数方法（覆盖外返回不可判定）并去掉门禁的区间聚合豁免 —— 那是 alert 求值路径的**真实行为变更**，属另一个 use case，不该夹进本片。（2026-08-18 由 user 拍板：本片保持形状豁免，此条记为后续待办。）

## 生产已知信号登记（2026-08-18 T013 取证期实测，不是故障）

上线当晚取证时实际触发过、且**今后还会周期性触发**的三条信号。登记在此以免下一个人当故障去追：

1. **宽区间 seed 在美股开盘前跑 → 必产生一条 us 交叉校验 WARN** —— 形如 `交叉校验不一致 {"market":"us","count":1,"dates":["<当日>"]}`。成因是时点而非分歧：历史段活源腾讯靠「指数当日有 bar ⟺ 当日开市」**反推**，ET 开盘前当日无 bar ⇒ 它说「今天不是交易日」，而前瞻段年历说是。T005 的留痕机制行为完全正确（不自动订正、两边数据都不动）。**要消除只需把 seed 挪到 ET 开盘后跑**，别去改判据。
2. **宽区间 seed 会让 us 心跳落 `tencent` ⇒ 探针红到次日 21:00** —— 富途历史上限约 10 年，`--from 2015-01-01` 触发 `FutuCalendarAdapter.assertNotTruncated` 的截头闸（阈值 2 个工作日），富途**自判失败**、链平移到 L2 腾讯。这是**真降级**、探针报得对；只是它由人工宽区间触发。心跳是「最后一次填充是谁服务的」**快照**，下一次填充（次日 21:00 cron，30 天窗不触发截头闸）自动写回 `futu`，**无需人工干预**。
3. **降级「频次」目前只在日志里** —— `calendar_sync_health.served_by` 答的是「最后一次是谁」，答不了「多久降级一次」。链每次平移都打 `falling through` WARN，频次在日志中是全的，只是没接进飞书告警路径。**本片故意不补**：要频次告警需新计数器 + 新告警链，而本仓观测面是 SQL 探针 + 飞书，代价与收益不成比例。真要做属另一个 use case。
