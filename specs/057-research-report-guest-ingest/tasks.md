...
- [X] T012 [Ops] **guest-proxy 新 location + env 管道**（`FR-013`, `FR-014`, `FR-016`, plan `D-6`）：`futu-shim-guest.conf.template` 加 `location = /research-report` —— `limit_except POST { deny all; }`（`FR-013`，两层各拒一次的通道那层）/ 市场闸 `$arg_symbol !~ "^(cn|hk|us):"` → 400（**注释必须写明为何与行情端点的 US-only 刻意不同**）/ `client_max_body_size 20m`（= 16MB 上限 + slack；全仓目前零处设置、默认 1m）/ **新开** `limit_req_zone guest_upload:1m rate=2r/m` + `burst=1`，且注释里**必须写明它与行情三个 zone 不同类** —— 行情 zone 护的是上游 vendor 官方限额，本 zone 护的是**宿主磁盘**（`proxy_request_buffering` 默认 on，body 整份先落盘，模板顶部注释写着「可用磁盘是几个 G 量级」；2r/m × 16MB × burst 1 ⇒ 峰值驻留约 48MB）/ **整组三条 `proxy_set_header`**，Authorization 换 `Bearer ${GUEST_UPLOAD_TOKEN}`（Guardrail 7）/ `proxy_pass http://127.0.0.1:3001/api/v1/research/reports`。配套：`docker-compose.guest.yml:52` 的 `NGINX_ENVSUBST_FILTER` 加新键（Guardrail 8）、`nvy-guest-proxy.env.example` 加 `GUEST_UPLOAD_TOKEN=__FILL_GUEST_UPLOAD_TOKEN__`、`render-env.sh` 按 `FUTU_SHIM_TOKEN` 那套加 `: "${GUEST_UPLOAD_TOKEN:?}"` + perl 替换（Guardrail 9）。→ verify: `nginx -t` 过 + 本机 dry-run `render-env.sh` 产物无 `__FILL_` 与 `${` 残留

- [X] T013 [Ops] **`verify-guards.sh` 反例断言** ✅ **2026-08-16 已在访客机实跑：`--from-guest` 40 条全绿**（首跑 39/1，两条缺陷已修：`GET` 期望值 405→403、超上限那发漏了 429 重试）（`FR-012`, `FR-013`, plan `D-6`）：新增断言，**每条配反例**：无 token POST → 401 / `GET /research-report` → **403**（`limit_except` 里干活的是 `deny`，返 403 不是 405 —— 2026-08-16 首次真跑证伪了原先写的 405）/ `symbol=us.PEP` 等非归一写法 → 400 / `symbol=hk%3A1698` 百分号编码 → 400 / 非 PDF → 422（**证明请求真的到了 app 而不是被 nginx 吞了**）/ 超上限 → 413 且能区分是 nginx 还是 server 返的 / `--from-guest` 新增 **loopback 端口在 wg2 地址上不可达**（plan `D-6` 那三条 iptables 推理的落地护栏 —— 没有它，将来有人把新端口加进 ACCEPT 规则时不会有任何东西红）。⚠️ `check()` 用 `%-46s` 对齐，标签别超 46 字符。→ verify: 本机 `./verify-guards.sh` 全绿（29 → 约 35 条）；prod 上 `--from-guest` 全绿（32 → 约 39 条）；**先把新断言跑红再实现**

- [X] T014 [Ops] **`SKILL.md` 上传章节 + frontmatter description** ✅ 2026-08-16 已在访客机装机验证。⚠️ 原写的「验 `source-origin.json`」是**错误假设** —— `openclaw skills install` 只落 `SKILL.md`，不写该文件；真正的自证是 `openclaw skills info nvy-futu-kline` 显示 `✓ Ready` + `Visible to model: yes` + 新 description 已生效（装前是「one endpoint」的旧版，装后是两段式）。另：本条原写的「顺带修 502 冷启口径 drift」经核对**仓内 canonical 早已是正确口径**（drift 只存在于 guest 机上那份 08-04 旧版），重装即消除，仓内无需改动（`FR-020`, plan `D-8`）：加上传节 —— 端点 / 三个必填 query 参数 / `-F file=@` 形态 / **symbol 的冒号不要 percent-encode**（`$arg_*` 不解码，编码会 400；方向 fail-closed 安全，但不写明模型会反复试错烧限频）/ 限频与体积上限 / 错误码对照表。🚨 **frontmatter `description` 必须改**（现写死「six endpoints」「US stock … or US options …」，不改则新能力对模型完全不可见）；**slug `nvy-futu-kline` 保持不改**（Guardrail 10），正文加一句历史名说明。顺带修既有 drift：把「502 = OpenD 冷启，等 30 秒是正常表现」那条作废口径改掉（#868 起 OpenD 常驻）。→ verify: `make-guest-bundle.sh` 跑通（4 条出包自证含 `__FILL_|TODO|FIXME` 扫描，Guardrail 11）+ 过仓内 skill 片段可跑性机器闸（`bash -n`，per #55）+ guest 机 `FORCE=1 ./setup.sh install-skill` 后 `source-origin.json` 指向 `services/guest-proxy/openclaw-skill`

- [X] T015 [Ops] **端到端实证**（`SC-001`~`SC-009`）—— 2026-08-16 从访客机（运维 Mac）实跑完成：**`SC-001`** 一条 curl 完成投递，无多步往返 ｜ **`SC-002`** 2,020,387 字节（真实样例，泡泡玛特深度研报）→ **201 / 5.49s**，门 30s ｜ **`SC-003`** 同字节重投返回同一 `reportId=1`、`deduplicated:true`、库里仍 1 行、对象未新增（第二发 1.24s，省掉的正是 OSS put）｜ **`SC-004`** 非 PDF 422 `RESEARCH_FILE_NOT_PDF` / 市场 400 / 冒号编码 400 / `GET` 403 / 超上限 **413 且响应体是 JSON ProblemDetail**（证明是 multipart 那层先跳闸而非 nginx）｜ **`SC-005`** `GET`/`DELETE` 通道层 403，服务端零读接口 ｜ **`SC-008`** 落库行四问可答：`hk:09992` / `2026-08-12` / `guest:friend1` / `2026-08-16 01:31:46+00`，且 `status=COMMITTED`、`size_bytes` 与本地文件逐字节一致、中文原文件名完整穿过 multipart ｜ **`SC-009`** 既有 6 条行情端点的全部断言仍绿（`verify-guards.sh --from-guest` 40/0，含三条正例真打上游 200）。⬜ **仅剩一项未做**：spec Assumptions 提到两份样例，本次只投了其中一份 —— 第二份非阻塞，投它只会再验一遍同一条路径。
...
**上线顺序** —— 通用规则（五条部署链的人工闸、SOPS 派生消费者、四问排序决策）已收口到
[`ops/runbook/deploy-topology.md`](../../ops/runbook/deploy-topology.md)，**本节只记 057 这次的实际经过**：

1. 带外推密钥到宿主：`GUEST_UPLOAD_TOKEN` + `RESEARCH_OSS_*` 三个新键。
2. 发版 → 批 `production` 环境 → app 带 `127.0.0.1:3001` 上线。
3. 在宿主上重渲染 `/etc/nvy-guest-proxy.env`（**首次引入新键必须手工把 `render-env.sh` +
   `nvy-guest-proxy.env.example` 送上宿主**，因为新版脚本本身只能靠那次 deploy 送到 —— 先有鸡
   还是先有蛋，见 [`prod-deploy-rollback.md`](../../ops/runbook/prod-deploy-rollback.md) 同名小节）。
4. 触发 `Deploy guest proxy` → 通道侧上线。
5. guest 机装 skill（T014）→ 端到端实证（T015）。

> ⚠️ **实际没按这个序走，502 窗口真的出现了**：`Deploy guest proxy` 在合入时自动触发并**先红在预校验上**
> （envsubst 正则漏改，见 #64），修好后通道侧先上线，而 app 侧还卡在 `production` 环境的人工审批上。
> ⇒ 顺序的下界由**人工闸被点的时机**决定，不是由触发方式 —— 这条已作为通则写进 deploy-topology.md。
