...
- [ ] T014 [Server] **合并用例 ②：事件合并编排 + 信号接线 + 提前消失 + 乐观并发 + 美股增量入口**（`FR-007`, `FR-012`, `FR-016`, `FR-021`, `SC-005`, plan §D8）：前置 = T004、T005、T006、T013、T029。同文件扩展：港股日常入口 —— 起手调 T029 为缺档案的港股锚反推财年；对当天有观测或信号变化的事件全量重算 + 逾期扫描 + 未知日期扫描（只扫 `market = 'hk'`，是否有档案交给 T005），把 T011 的 120 天信号、事件既有确认日期与口径、「是否曾有清单观测」、交易日数交给 T004 / T005；据上一轮清单集合（该来源观测的上一次最近出现时刻）与 T012 回传集合判定提前消失（仅本轮清单成功时）；每事件一事务（事件 `updateMany where { id, revision }` → 流水 insert），命中 0 行重读重算最多 3 次；🚫 `FOR UPDATE`；间隔更新写 `earnings_meeting_lag`。另导出**增量入口**：只算传入的 `(instrument, period_key)` 集合（美股钩子 T020 调用）。→ verify: 079 IT：① 会前通知刊发后、清单首次列出该期那一轮 ⇒ `confirmed`、确认日期 = 通知刊发日而非首次观测（`SC-005`，US1 AS1）；② 两精确口径冲突 ⇒ `conflict` + 全部日期可查；次日一致 ⇒ 解除留痕（US2 AS2 / AS4）；③ 通知在第 1 天、富途日期第 30 天才出现并在第 40 天改期 ⇒ 每轮确认日期都 = 通知刊发日（排序铁律 4）；④ 两次合并交错写同一事件 ⇒ `revision` 冲突重试后结果正确；⑤ 增量入口传入 2 个美股键 ⇒ 生成 2 个 `unconfirmed` 事件；定向变异：去掉 `revision` 条件 ⇒ ④ 必红；T011 信号窗口改回 7 天 ⇒ ③ 必红。（`state_branches` 4、9、10、16）
