...
- [ ] T018 [Server] **079 场景 IT ②：状态迁移（经维度运行）**（`FR-016`, `FR-017`, `FR-019a`, `SC-011`, plan §D12）：前置 = T016。同文件经维度执行：① 满 2 个交易日未刊发 ⇒ `overdue` + finding 且未转历史、该轮 `sync_run.status = partial`，次一轮仍逾期 ⇒ `success`，注入刊发 ⇒ 解除转 `published`（`SC-011`；种 `trading_day` 含假日与 `unknown` ⇒ `unjudged`、`success`）；② 两份 fixture 页先后运行、同期会议日变化 ⇒ 改期流水；③ 第二份页缺该行、会议日未到 ⇒ 提前消失 finding、确认保留，再插一轮失败页 ⇒ 不判消失；④ 通知信号 2 个交易日后仍无日期：曾在清单的标的 ⇒ `notified_undated` + finding + 该轮 `partial`，从未在清单的创业板代码 ⇒ 仅计数、`success`。→ verify: 四臂全绿；定向变异：逾期改按日历日 ⇒ ① 必红；提前消失在解析失败轮次也判 ⇒ ③ 失败轮断言必红；未知日期标红不查「曾在清单」⇒ ④ 创业板臂 `success` 断言必红。（`state_branches` 11、14、15、16；Edge 3 / 4 / 10 / 11）
