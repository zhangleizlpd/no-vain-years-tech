...
## 修复记录（2026-09-16，维护者批准「都修」）

| ID | 修法 | 落点 |
|---|---|---|
| G1 | verify 末尾补交叉引用：本条谓词的「能红」证明在 T010 定向变异 a；同时写明迁移属「最终状态」形态、反例构造不出来，🚫 为它单造一个永不会红的断言 | tasks.md T009 |
| T1 | 臂 ③ 补一次术语对应：spec 与 plan 称「事件源重启」，在本片即指 shim 进程重启 | tasks.md T003 |
| D1 | 标题括注补 `plan D9`（原只写 `plan D1`） | tasks.md T001 · T003 |

修复后复核：plan D1–D11 全部被至少一个 task 的 `plan Dx` 标签引用，D9 不再落空；三处均为一行级编辑，**未改变任何 task 的实施内容与验收臂**，故覆盖预检五张表与 Metrics 不变。
