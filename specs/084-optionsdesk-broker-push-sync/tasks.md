...
- [ ] T012 [Server-IT] **`kind` 隔离回归：开盘前对账不受补偿影响**（FR-010; plan D5; state_branches 10; US3）：本 task **不改生产代码**（隔离本就成立，起片前已取证：`broker-account.scheduler.ts:247` / `:251` 的 `scope` 与 `schema.prisma:2413` 的索引谓词都带类型过滤）—— 只补回归断言，防后续改动悄悄破坏它 → verify: `pnpm nx test server test/integration/optionsdesk-084.push-gap.it.spec.ts --skip-nx-cache` 先红 → 绿，臂：① 🚨 当日**已有一条成功的缺口补偿**，开盘前对账时点到来 ⇒ **照常发起**（branch 10；把补偿误记成对账类型的实现在此红）② 当日补偿 + 对账各一条 ⇒ 按对账类型统计当日仍恰 1 条 ③ 补偿记录**不**撞对账的部分唯一索引 ④ 对账的「当日是否已成功」判定不把补偿计入；定向变异：把补偿改写成对账类型 → ①②③ 同时红（留档）
