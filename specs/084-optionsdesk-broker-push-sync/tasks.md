...
5. **防重入索引谓词与上游不同**：只含**执行中**状态，**且只罩 `kind='gapfill'`**。🚫 照抄上游把 `succeeded` 也纳入 —— 断档一天可能合法发生多次，纳入会让当天第二次断档**再也补不回来**且静默。🚫 把每 5 秒一次的推送刷新（`push`）一并罩住 —— 刷新与补偿会互相阻塞，表现同样是**持仓静默不刷新、不报错**（084 impl 期决策订正：`kind` 本片加的是 `push` / `gapfill` **两个**取值）。
