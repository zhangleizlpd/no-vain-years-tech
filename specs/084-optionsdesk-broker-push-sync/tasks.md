...
- [ ] T011 [Server-IT] **卡死回收补第三条分支**（FR-011; plan D6; state_branches 7; US2）：`reclaimStuckRuns`（`broker-account.scheduler.ts:117-136`）现只处理 `backfill`（置回 `pending`）与 `reconcile`（置 `failed`）⇒ 加新类型分支，语义取 `reconcile` 那侧（置 `failed` 并计入当日失败次数，由重试规则重新发起） → verify: `pnpm nx test server test/integration/optionsdesk-084.push-gap.it.spec.ts --skip-nx-cache` 先红 → 绿，臂：① 构造一条超时的执行中补偿记录 ⇒ 被回收为终态（branch 7）② 回收后计入当日失败次数，重试规则据此重新发起 ③ 未超时的执行中记录 ⇒ 不被回收 ④ 既有 `backfill` / `reconcile` 回收行为不变（回归）；定向变异：不加第三条分支 → ① 红且记录永久停留在执行中（留档）
