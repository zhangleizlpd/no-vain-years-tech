...
- [ ] T017 [Ops] **上线后观察：开盘前对账未被污染**（SC-004; state_branches 10）：推送上线后累计 5 个交易日，每个市场每个交易日仍恰有 1 条成功的**开盘前对账**记录，且无一个交易日的对账因当日发生过缺口补偿而未发起。统计 SQL **按开盘前对账类型过滤**（本片引入第三种类型后不过滤即误判）→ verify: 观测值回填本行；**开 task 时同步建 issue** 写明触发条件与兜底复查点；与上游 T022（issue #427）**同期进行、判据独立**，两者都按类型过滤故互不干扰
