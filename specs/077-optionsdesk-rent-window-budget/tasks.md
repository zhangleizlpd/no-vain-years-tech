...
| mobile e2e mock 工厂（7 处，typecheck 逼出）      | `apps/mobile/e2e/optionsdesk-{march-audit,intraday-tiers,offline-ladder,chain-report,detail-thermometer,chain-leg-picker,query-pushdown}.spec.ts` |
...
10. **新增 public 字段必 grep 三类手写镜像**（#379）—— 本片已查实：contract-smoke 顶层闭合键集 + 两份 golden JSON 基线**都逼不出 typecheck**，`nx affected` 绿不代表它们绿。
...
| SC-006 每条 branch 能被变异证红   | T001 / T002 / T003 / T004 / T005 变异留档         | 变异留档                          |
...
| EC2 上界之下一档都没有      | T001-⑧（`strikes` 空 ∧ `trimmed = 0`）                                   |
...
| US3-AS1 真无合格腿 ⇒ 空而非凑         | T001-⑧ + T008 SQL ②                      |
