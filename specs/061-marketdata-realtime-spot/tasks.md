...
- [X] T018 [Ops] **端到端实证**（`SC-001` / `SC-002` / `SC-003` / `SC-005` / `SC-007`，**唯一载体**）：美股盘中真机（Mate50）逐条实测并把数字填回本 task —— ① 距 W% 分钟级跳动，端到端 P95 按 `intraday_at` 与页面读数时刻实测（`SC-001`）② 收盘后 2 分钟内的价 == 当日官方收盘价，且此时 `sync-anchor-quote` 的每小时投影尚未跑过（`SC-002`）③ 人为断源 → 三轮后全部回落收盘档、**0 个锚显示 0**（`SC-003`、`SC-007`）④ 恢复源 → 自动回升，无人工介入 ⑤ 一个交易时段内的实际调用量占配额比（`SC-005`）。🔀 **原 ⑥（`SC-006` 连续 3 个美股交易日 + 失败轮次逐条可查）已于 2026-08-18 拆出本 task，改由 issue #98 承载** —— ①～⑤ 一场夜盘即可跑完，⑥ 物理上要 3 天；把一个已在 prod 跑着的 feature 卡三天，代价与信息量不成比例。**判据一字未改，改的只是由谁承载**。→ verify: 逐条记录实测值（数字 / 截图路径 / `failstreak` 留痕）填回本 task；`SC-004`（alert 零变化）由 T005 的 diff 断言 + 024 既有 IT 全绿承担，不在本 task 重复
  ...
  **2026-08-18 美股夜盘实测**（prod `server-v0.31.0` / mobile `0.11.0` build 13 / 11 只非排除锚全为美股 / `calendarBasis = confirmed`）：
  ...
  - 失败留痕原件：`docs/private/evidence/061-sc006-intraday-collection/outage-2026-08-18-failstreak-trace.log`（20 行，失败轮次 1→6 逐条编号 + 熔断 + 回升；**本机私有，未公开**）。2026-08-25 从随 session 死的 scratchpad 迁入工程私有区并纳入每日备份，见已关闭的 issue #98。
...
| SC-006 | **issue #98**（2026-08-18 拆出 T018）✅               | 连续 3 个交易日无人工干预 + 失败轮次可查 —— 判据未改，换载体。**2026-08-25 收官**，见下 |
| SC-007 | T010 · T015 · T018（③）                               | 熔断全过程无一刻显示 0 或空白                                |

> **`SC-006` 收尾（2026-08-25）**：三个美股交易日（US 08-20 / 08-21 / 08-24）各 **1431 / 1434 / 1465** 拍，**0 处 > 60 s 空档**，`failstreak ≠ 0` 与 `circuit ≠ closed` 全程 **0 拍**，零人工介入 ⇒ 判据**前半句**成立。
> 🚨 **后半句「失败轮次可从留痕中逐条查出」的能力证明不在这三天** —— 三天里根本没有失败轮次发生；它由 T018 ③④ 的人为断源留痕承担（`outage-2026-08-18-failstreak-trace.log`，失败轮次 1→6 逐条 + 熔断 + 回升，且是从归档日志里查出来的）。**两者合起来**才构成这条判据，单靠任一半都不成立。
> **要重跑这条**（改了 scheduler 拍频 / 熔断参数 / 实时源 fallback 链 / 部署方式时）：采集 + 看护 + 分析脚本都在 `docs/private/evidence/061-sc006-intraday-collection/`（**本机私有，未公开**；起法见该目录 `README.md`），资产清单与「后续每场怎么保证」写在 issue #98，不必重写。
