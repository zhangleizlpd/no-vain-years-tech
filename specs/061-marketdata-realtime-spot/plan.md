...
#### 🚨 「一行不动」的射程订正（2026-08-17，T011 实装后）

上面三条理由针对的**全部是它的写**（`breach_started_on` 的日粒度转换）。但 `advanceBreachState()` 顺带还返回两个**纯展示计数**（`baseTotal` / `actionableTotal`），它们不写库、不进 DTO，唯一去向是空态判定 —— 把「一行不动」按字面读到这两个数上是**读过头了**，且代价具体：

- `actionableTotal` 判 `belowW` 用裸 `last_close`，而 T011 已把筛选 / 排序 / 距 W% 全换成 spot 表达式；
- `all_idle` 在 mobile 侧是压在**非空列表**头上的 `ListHeaderComponent`（列表为空时先被判成 `filtered_empty`）⇒ 横幅与行**必然同屏**；
- ⇒ 盘中出现「新鲜实时价跌破 W、收盘价未跌破」的锚时，横幅说「一个都没有」、底下的行是红色负距 W%。**同一份响应里两个口径回答同一个问题**，正是 D4 末条禁的形态，且**没有任何断言会红**。

**订正**：「一行不动」= 不改它的**写库判据与状态转换**；那两个纯展示计数**不在禁令内**。修法（T019）在**调用方**另发一条 `COUNT(*) FILTER (...)` 覆盖，函数体仍零行改动，两全。

📌 这条订正本身是个样本：**守门写的是「不许动什么」，而不是「为什么不许动」时，射程就会被读歪** —— 歪向宽松会破坏语义，歪向严格会留下这种没人敢修的缝。
