...
2. ~~回显标的名称排除在本片之外~~ —— **2026-08-16 已由用户决定纳入**。落定为 FR-012～FR-016 + FR-024/FR-025 边界 + SC-008（含盲区声明）。跨上下文读取的三档取舍已选定：实时只读直查（Q7-B），依据是同形态先例（预警上下文读同一份行情标的目录，per ADR-0052）+ 无摊销对象（判据同 ADR-0048 复审记录）。
