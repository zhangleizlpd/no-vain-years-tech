...
### T17 真机复验（2026-08-22，Mate50 dev-client）

窄视口取证之后，用主 worktree 已有的 dev-client APK（8/14 构建，065 未动原生层，直接复用）在 **Mate50** 上跑通了全链路真机验证。

**设备度量**：`Override size 1088×2400` / `density 520` ⇒ CSS 视口约 **335×738**（比探针里最紧的 375×667 更窄、比 360×780 更矮）。

| 验证点 | 真机结果 |
| --- | --- |
| **首屏可视行数（美股页签）** | **4 行完整可见**（KBR / LKQ / LULU / VICI，第 5 行 TAP 被 FAB 压住）—— 与探针 360×780 的 4 行**吻合**，结论不变 |
| 市场页签选中态 | 底部横条 + 字重双重编码，`react-native-web` 不渲 `aria-selected` 那条纪律在真机上无影响 |
| **FR-016 跨页签小圆点** | 切到港股后，**美股页签出现小圆点且不含任何数字**（美股有可动锚 KBR −9.5% / LKQ −3.7%） |
| **FR-012 港股说明常驻** | 港股**零锚时**在；**建了一只 hk 锚之后仍在** —— 这正是「MUST NOT 只在空态呈现」的正面实证 |
| 第 4 空态 | 港股零锚时渲「这个市场还没有锚 —— 换个市场看看」，且**零按钮**（无建锚 CTA、无清除筛选） |
| **FR-014 建锚白名单正向** | 选择器搜 `00700` 回「腾讯控股 · hk」**可选**，一路建成锚；DB 落 `id=14 ticker=hk:00700 **market=hk**` —— T02 单点派生 + T03 `ck_anchor_market` 端到端跑通 |
| **T12 失效链（既存 bug）** | 建锚后回雷达，新锚**直接可见** —— 失效链若仍断着会被 30s `staleTime` 卡住看不到 |
| 行级降级 | 港股锚未被采集 ⇒ 显示「行情不可用」，距 W 呈 **「—」而非 0**（FR-017 禁伪造） |
| 锚管理页 | **不分市场**、12 只全列（spec Clarifications Q1 的前提成立） |

⚠️ **两处环境事实**（不影响结论，但值得记）：① 机上原装的是 **standalone 0.12.0**（`flags` 无 `DEBUGGABLE`）—— 跑内嵌 JS、无视 Metro，必须换 dev-client 才看得到改动，换装需跨 keystore `adb uninstall`；② dev 库现有 13 只锚（12 us + 1 hk），本次新建的 `hk:00700` **刻意保留**，让后续 065 相关验证在港股页签上有数据可看。

⚠️ **窄视口取证的原始记录**：Mate50 已 ADB 连线，但机上装的是 **standalone 0.12.0**（`flags` 无 `DEBUGGABLE`），会跑内嵌 JS、无视 Metro ⇒ 看不到本片改动；换 dev-client 需跨 keystore `adb uninstall`，会抹掉机上该 app 的本地数据与登录态，故未擅自执行。窄视口是 T17 verify 明列的等价路径；真机复验（含 status bar / 手势条的额外占用）建议在下次装 dev-client 时顺带做一次。
