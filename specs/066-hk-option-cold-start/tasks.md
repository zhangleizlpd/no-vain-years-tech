...
- [ ] T15 [E2E] **真港股锚跑通整链**（`SC-001`, `SC-003`, plan §Gate 0.1）：**前置 = `T06` / `T07` 已部署到 prod**（**不是**隧道 —— 见 T14 的撤销说明）。用 **prod** 的 `/anchor-import` 建一只真实港股锚（该 command 原生支持 `hk:`，nginx 那道闸也是 `^(us|hk):`），随后逐条查 **prod 的库**。
  ...
  - 📌 **2026-08-26 范围收窄 + 实验设计（prod 只读取证后写下，动手前）**
    ...
    **预注册判据**（先写死，事后不许改口径）：`outcome = backfilled` · `target_session` = 当日 · `source = eod` · **`oi_as_of` = 当日**（若为前一交易日 ⇒ 分叉没生效，这是核心判别）· 合约数与快照行数相等且 > 0 · `SC-003` 覆盖标的数 **N: 28 → 29** 且仍远小于 hk universe（🚨 **2026-09-07 订正：原写「3 → 4 / 2791」已陈旧** —— 那是 2026-08-26 全仓仅 3 只港股锚时的数。**「覆盖标的数」= 锚数 N**（spec.md:274「覆盖的标的数恰为 N」），**无挂牌期权的锚照样计入 N、MUST NOT 排除** —— prod 实查近三轮 `sync:hk_option_contract` 均 `scanned = 28 ∧ ok = 28 ∧ skipped = 0 ⇒ ok+skipped = 28 = N`，当前实现把无链锚记在 `ok` 里。hk 锚 28 只、hk universe **2793** 只。🚫 **MUST NOT 拿「有链锚数 22」顶替 N** —— 那是另一个指标（076 T009 判据② 才用它），换指标对拍会得出一个看起来合理的错结论）。⚠️ 若执行滑出窗口导致 `source = premarket_backfill`，本次**作废而非失败**，改日重跑。
