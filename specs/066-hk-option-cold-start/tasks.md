...
- `[P]` = 可与相邻 task 并行（不同文件、无未完成依赖）。本片的并行组：**T01 / T02 / T03 三者互不依赖**（各改各的文件，彼此无序约束 —— 约束只存在于 T01→T04、T02→T04、T03→T06 这三条**下游**边）；**T07（标的 IV）与 T09（实时报价）彼此独立、也独立于 T01–T06 那条链**；T11 独立于全部 server task。⚠️ T12 依赖 T11（e2e 断言要等 rules 改完），**不是**并行对。
...
| U2 采样器（**仓外**，一次性取证） | `broker-hk:/home/admin/nvy-u2/` —— 收尾必删，见 T12 |
...
4. **T08 是 P1 故事的真正终点，且它被 U2 卡着**：在 HKEX 的 OI 归属日实测出结论之前，`COLD_START_CAPABILITY.hk.optionSnapshot` 保持 `false`、`hk_option_daily_snapshot` 保持 `enabled = false`（spec `FR-016`）。⇒ **T01–T07 可以照常推进并各自合入**，只有快照那一档等 08-25。🚨 别为了"让 SC-001 早点绿"提前翻这两个开关 —— 归属错了不报错，只让持仓量整体偏一天。
5. **T09 的四处改动必须同一个 task**（含港股午休还原）。漏第三处 ⇒ 午休盘口被当成盘中价写进锚表，雷达照常渲染、排序照常成立、**没有任何断言会红**。
...
| SC-011 | T05 |
| SC-012 | T07 |
| `state_branches` 1–21 | 1 → T06 ｜ 2 → T06 ｜ 3 → T06（午休，T09 还原两段后仍成立） ｜ 4 → T06 ｜ 5 → **T03** ｜ 6 → T06 ｜ 7 → T06 ｜ 8 → T05 ｜ 9 → T05 ｜ 10 → T02 ｜ 11 → T02 ｜ 12 → T02 ｜ 13 → **T02**（美股等价性，该 task 的核心断言） ｜ 14 → T07 ｜ 15 → T07 ｜ 16 → T09 ｜ 17 → T09 ｜ 18 → T09 ｜ 19 → T09 ｜ 20 → T10 ｜ 21 → T08 |
| **Acceptance Scenario 11 条** | US1: AS1→T06+T13 · AS2→T06 · AS3→T06 · AS4→**T03**（seed 资格那条，T13 端到端复核）；US2: AS1→T07 · AS2→T07 · AS3→T07；US3: AS1→T09 · AS2→**T09**（午休） · AS3→T09（半日市） · AS4→T11+T12 |
| **Edge Case 10 条** | 无挂牌期权→T05 ｜ 有期权但快照未落→T05 ｜ 两者不得折叠→T05 ｜ 希腊值缺失→T08 ｜ 港美同时触发→T10 ｜ 停牌→T01（`suspension` 字段原样透出，不作跳过依据） ｜ 两地上市→T01（`stock_owner` 关联，两只独立证券） ｜ 日历视野未覆盖→T06 ｜ 建锚回滚→T06 ｜ 重复投递→T06 |

> 📌 **本表尚未经 `/speckit-analyze` 复核** —— 上面的映射是写 tasks 时人对着 FR 展开的产物，而本仓实证过 **SC 层是系统性盲区**（045 那次同一份 tasks 里 FR 37/37 而 SC 仅 6/11，漏的恰是「只有口号、没有验证手段」的那两条）。analyze 阶段请**实时 `grep` 重数**，别抄这张表。
>
> 🚨 **写这张表时已经栽过一次，留在这里当证据**：`state_branches` 那一行初稿从第 9 条起**整体错位**（把 13「美股等价性」错标给 T07、把 21「希腊值缺失」错标给 T05），成因正是「凭记忆对着写」而不是把 21 条真的枚举出来逐条对。改正靠的是把 frontmatter 的 `state_branches` 用脚本编号打印出来比对。⇒ analyze 阶段**必须重跑这个枚举**，本表的正确性不构成它下一次仍正确的证据。
>
> 📌 **一处预期的弱覆盖**：`SC-005`（开通港股前后美股各维度覆盖的标的集合逐元素相同）落在 T02 的等价性断言里，但它断的是**工作集判据**这一层；「美股各维度**行为**不变」的全量证据实际由 `pnpm nx affected` 的既有 server 测试套承担。analyze 若判它覆盖偏薄，**那是准确的**——补法是在 T02 加一条跨维度的集合快照对比，而不是新起一个 task。
