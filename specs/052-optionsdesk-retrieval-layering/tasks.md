...
- [X] T016 [Gate] **七项标定 + 写回 spec**（`SC-011`, plan `D-CALIB-1`）：用 dev 全部 12 条链，沿 `050` T017 的直方图找谷底 / 衰减终点做法，标定：成色兜底比例 X · 流动性档界（**相对与绝对价差两个口径都要评**）· 活跃标绝对下限 · 分层降级阈值 · 费率打平带宽 · 召回候选上限 K · 是否设单笔权利金下限。→ verify: 七项数字与**推导过程**写回 spec § Assumptions；代码内**零处**未标定的占位常量（`rg` 扫 `TODO|占位|placeholder` 零命中）；🚫 若某项分布无明显断点则**记为「不设该条件」**而非拍一个数

  🚨 **动手第一件事就把数据面判死了 —— dev 最新一期是 mock 数据**。按「每票取自己最近一期」导出后，12 只票**共用同一个 spot `128.40`**、整条链每个行权价共用同一组报价（`2026-08-12`：3617 行只有 **2 个**不同报价元组，而前一日是 7025/7063；`quote_as_of` 落在 `08-13 00:11`）。⇒ 若不看这一眼，我会拿「ACN 现价 128.40、K=70 的两周 put 报价 2.30」去标七个策略参数，**再把它们当「实测标定」写进 spec** —— 数字全都出得来，此后没有任何一处会红。**user 裁定：改用 `2026-08-11` 那一期（最近一期健康数据，与 `050` T017 的 `08-10` 口径直接可比）；污染只登记不在本片处置。**<br>📌 **待办（不在 052 内）**：dev marketdata 同步在 `08-12` 那轮写了 mock 数据，需查 `MARKETDATA_PROVIDER` 运行态与 09:05 定时任务日志（`ops/runbook/scheduled-tasks.md` 故障排查段）。

  🚨 **一条方法论上的发现，已写进 spec 与两处常量注释：两类参数的判据不同，不能都去「找谷底」**。**阈值**（成色 X / 活跃标绝对线 / 单笔权利金下限）是「在/不在」的二分 ⇒ 谷底是唯一诚实的判据，找不到就**不设**；**档界**是把连续量**分箱** ⇒ 分位切分本就是标准做法。这条要写下来是因为流动性档界的三刀里**只有最低那一刀有谷底**（活动量对数密度是单峰 + 一个死腿尖峰），若不点破，下一个人会把「另外两刀没有谷底」当缺陷去补。

  📌 **七项结论（逐项判据与全部读数在 spec § 标定实测）**：成色兜底 X **`0.04 → 0.03`**（两模间隙 `[2.09%,3.91%]` 中点）· 打平带宽 **`0.02 → 0.01`**（判据换成「实际翻转了多少对顺序」，收租 `20.9% → 12.0%`）· 单笔权利金下限 **裁定不设** · 其余四项（档界 / 活跃标绝对线 / 降级阈值 / K）**维持原值但理由全部换成实测的**。

  🚨 **两项「维持原值」其实是发现，不是无事发生**：① **降级阈值的数据只给得出区间不是点** —— 逐票候选数有大缺口（收租 `…8,9,9` → `27,59,92`），`[10,27]` 内任何取值在本数据面上行为**逐字相同**；取下沿 `10` 靠的是方向性理由（分档是功能、降级是保险）+ 下界约束（4 档 × 每档 ≥2 条 ⇒ ≥8）。② **活跃标绝对线 `100` 换了理由** —— 原本是「借用 `LIQUIDITY_TIER_BOUNDS` 第二档界」，现在它自己有谷底（组内 top-1 活动量双峰之间 `[46.4,215)` 密度 9 的平坦谷）。⇒ 两个 `100` 现在是**巧合**，注释已写明 🚫 不许把其一改成引用另一个。

  🔬 **`X` 改动的爆炸半径实测精确到 3 条腿且逐条同源**：收租候选 `223 → 220`，少掉的全是 `us:ARE K=50.00`（`+3.91%`）的三个到期日；全腿 `1592` / 建仓 `149` **逐条不动** —— 正是 `FR-006`/`FR-007`「成色只作用收租」的实测见证。

  ⚠️ **`X` 改值把一条既有断言的余量压薄到 0.3，已就地加固**：`leg-recall.rules.spec.ts`「收窄但零排除」原用 `strikeMax = 113` 对默认上界 `114.4`（余量 1.4），`X=0.03` 后默认上界变 `113.3` ⇒ 余量只剩 **0.3**。它仍绿、仍在测对的东西，但下次动 `X` 时会**静默换掉自己测的东西**（从「收窄」翻成「放宽」，两种情形下断言都绿）。⇒ 改用 `106`，离最高存活腿与默认上界各留 `1 / 7.3`。

  ⚠️ **T016 verify 里「`rg` 扫 `TODO|占位|placeholder` 零命中」这条判据本身不可执行** —— 本仓「占位符」是 UI 常用词（空框占位 / 缺值占位 / `placeholderTextColor`），裸扫 60+ 命中全是假阳性。**实际执行的判据 = 扫 `⏳` 与「标定在 052 T016」「MUST NOT 当已标定值引用」三个标记，均零命中**。这条与 T015 那处 plan/tasks 冲突同类：判据写得不可执行时，MUST 换一个等价且真能跑的，🚫 不许放着不跑还打勾。

  ✅ verify: `nx test server` **411 文件 / 4095 用例绿** · 五条守门全过（`check-optionsdesk-rule-constants` 八不变量 —— `0.03` 在扫描面实测零子串命中 / `check-test-size` / `check-server-moat` / `check-api-property-nullable` / `check-contract-smoke-drift`）· 占位标记三种扫描均零命中 · 七项数字与推导过程已写回 `spec.md § 标定实测（T016，2026-08-13）`。<br>⚠️ **`nx run mobile:contract-smoke` 本轮未跑** —— 它要求 `:3000` 空出，而该端口上是**你正在用的 dev server**（`node dist/main.js`，`nx serve server` 的 watcher 下）。本片契约与客户端零改动、且 T014 的断言蓄意不碰阈值取值，风险低但**不等于已验**。
...
| 单笔权利金下限 | **不设** | **不设** | **不设** | T016 已裁定「不设」（分布无切口 + 与 spot 相关 −0.742） |
