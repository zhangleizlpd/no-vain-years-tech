...
**建仓视角零判据改动**（`FR-007` / `SC-005`）：IT 断言本片前后候选集逐条相同。
...
24 条 `state_branches` 逐条 `it()`。重点断言：成色条件的三条边界（高于 / 恰等 / 稀疏网格兜底）· 死腿的免死条款（OI=0 但有成交仍进）· **建仓候选集本片前后逐条相同**（`SC-005`）· **被排除的腿在全腿视角可达**（`SC-006`，`051` 回归防线）· 排名基准随检索条件变化（`FR-026`）· 分层与降级的边界。
