...
- [X] T013 [Ops] **美股盘中真机实证**（`SC-001`, `SC-002`, US1-AS1, US1-AS2, US2-AS1, US2-AS2, US2-AS3, US2-AS4, US3-AS1, US3-AS2）：美股常规时段（北京 21:30–04:00）真机打开选约表，逐条与外部行情终端对拍 bid/ask/成交量；下拉刷新观察 asOf 推进；记录**候选范围最大那只**的端到端等待时间（2026-08-19 实测是 `us:ACN`，265 合约；`SC-002` 预算 P95 ≤ 1.5 s，基线是 p0 线上实测的 0.35–0.41 s）。⚠️ 这三类 web e2e **验不到**（spec `web_compat_notes` 已写明）：真实时段内候选集随价格移动的进出 / 收盘那一刻的档位切换 / 与真源的数值一致性。→ verify: 截图 + 对拍表贴进本文件；env-gated 真 vendor IT（`RUN_MARKETDATA_IT=true`）手动跑一次并贴输出（该门恒 skip，「测试全绿」对真契约不构成证据）
  ...
  **补录（2026-08-21 21:30 → 08-22 04:30 CST / prod `server-v0.33.2` / 真机 CET-AL00 · mobile 0.12.1）** —— 目的有二：① 补上首轮登记为「本轮未观测」的**收盘那一刻**；② 覆盖 #124（实时独载基线）合入后新长出来的那条路径，首轮实证时它还不存在。

  **A. 盘中新建锚全链（`us:ADBE`，首次在真机上走完 建锚 → 冷启动 → 腿表）**

  | 时刻 (CST) | 事件 |
  | --- | --- |
  | `23:22:24` | 真机点「保存」建锚 |
  | `23:22:30` | outbox published → BullMQ 入队 |
  | `00:44:02` | 冷启动 job 实际起跑 —— **排队 81 分 32 秒** |
  | `00:45:03` / `00:46:04` | 合约 0 → 1810 → **2304** |
  | `00:47:06` | **腿表 189 行 · `来源 realtime` · `● 实时 00:47:06`** |
  | `00:51:10` | 冷启动收 `intraday_skipped`，队列清空 |

  同一时刻 DB 内 `option_daily_snapshot` **0 行** ⇒ 这 189 行**完全由实时批自己撑起**，正是 #124 那条「库内没有 ⇒ 这一批实时自己就是基线」。冷启动一旦起跑 **3 分 04 秒**即可读；**81 分钟全花在排队**（冷启动 job 与夜间全市场批共用一条 `concurrency=1` 队列，纯 FIFO）。
  ...
  **B. 持续性（2 小时 21 分钟）**
  ...
  | --- | --- | --- |
  | 建仓视角行数 | 79 | **82** |
  | 来源 | `realtime` | `realtime` |
  | 未取到实时 | 0 | 0 |

  报价确实在动（`280P 19.15 → 19.10`、`270P 14.00 → 13.85`），成员提示报 `本轮 6 条新进 · 5 条已不满足` —— 候选集在盘中**动态**变化，正是 064 立项要的东西。全程 `option_daily_snapshot` 恒 0 行。

  ⚠️ **一个容易读错的数**：ADBE「0 条未取到实时」**不等于**覆盖比老锚更好（同晚 `us:ACN` 是 13/36）。差别来自 #124 自己的防静默纪律「没拿到实时值的腿整条丢弃」⇒ 实时独载基线下「没问到价的腿」不是标出来、而是**不出现**。**0 是设计使然，不是全都取到了。**

  **C. 收盘切换 —— 首轮「本轮未观测」的那一条，部分补上**

  `04:30`（美股 `04:00` 收盘后 30 分钟）实拍：区块头 `无数据时点 · 共 0 行`，档位条 `● 未就绪　这一批没有可用的报价时点 —— 不拿任何时刻顶替，下拉可重试`。

  ⇒ **没有拿盘中最后一刻的实时价冒充收盘价**，#124 写的「四条整体回落路径在实时基线下一律落回未就绪」实测兑现。

  🚨 **但要如实登记这条只补了一半**：观测到的是**收盘后 30 分钟的稳态**，不是**切换那一刻**（需值守或无人值守采集，仍未做）。且这一条走的是**实时独载基线**的回落路径（落「未就绪」），与首轮那句「收盘那一刻的档位切换」原本指的**有收盘档基线的老锚落回 `eod` 档****不是同一条路径** —— 后者至今未观测。

  **D. 本次补录新发现（均不属于 064 交付范围，已开 issue）**

  - **#139** 盘中建锚要等 81 分钟才开始拉腿（冷启动 job 在纯 FIFO 队列里排在夜间全市场批之后）；**并附空态文案 `chainNotReady` 已过时** —— 它写「采集在每个美股交易日收盘后跑一轮，下一轮覆盖到它就能读」，而实际是冷启动当场触发、且读到的是 realtime 独载基线，从没被快照覆盖过。
  - **#140** 收盘后实时独载基线整表清空，却同屏报 `本轮 80 条已不满足` —— 与紧邻那行「这一批没有可用的报价时点」自相矛盾（没有报价时点，判据无从评起）。是 #124 与 #130 的交叉地带。
