...
| **写任何涉及「今天」/ 交易日 / 剩余期限 / 折年化 / 陈旧判据 / vendor 时间戳解析的代码**                                      | `docs/conventions/cross-timezone-date-semantics.md`（四条时间轴 + 30 秒速查表「该调哪个函数」+ 「今天」跟谁走的决策表 + 6 问自检；决策与业内依据在 [ADR-0066](docs/adr/0066-time-semantics-ubiquitous-language.md)，机器强制 `scripts/checks/check-time-semantics.ts`。这类偏差**不报错**，只让数字悄悄差一天） |
...
[specs/064-optionsdesk-intraday-leg-quotes/plan.md](specs/064-optionsdesk-intraday-leg-quotes/plan.md)
