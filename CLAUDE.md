...
| **新增主机 / 服务 / runbook / env var 时判「这信息能不能进仓」· 写任何含 IP / 云账号 / 实例 ID 的内容 · 改标识符守门 · 写 fixture / SDD 文档 / PR 时涉及真实账户、持仓、交易数据** | `docs/conventions/information-boundary.md`（三层归属 + 代号纪律；`check-identifier-boundary.ts` 强制）                                                                                                            |
